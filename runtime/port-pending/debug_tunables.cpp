...
// ways to do that were an operator playing to it or CW_FAKE_PRESS_SEQ manufacturing
...
// the normal state until a level is actually running. That is why `CW_AUTOCHUCK` is
...
// How many screen requests have actually reached the frontend. `CW_FAKE_PRESS_SEQ`'s
...
// here; `CW_AUTOCHUCK_CLOSE_HASHES=hex,hex` overrides the list and an empty value disables
// the whole mechanism.
//
...
static long long AutoCloseDelayMs()
{
    static const long long v =
        getenv("CW_AUTOCHUCK_CLOSE_DELAY_MS")
            ? strtoll(getenv("CW_AUTOCHUCK_CLOSE_DELAY_MS"), nullptr, 10) : 1200;
    return v;
}
...
// `CW_FAKE_START_MS`'s own lines, so the two can be read against each other directly.
...
// CW_AUTOCHUCK=<state> — hand Chuck to the AI as soon as a level is running, with no menu
// navigation at all.
//
// The F4 debug menu can do this, but only through the HOST overlay, which is driven by SDL
// keyboard events — so it is unreachable from a headless run and from `CW_FAKE_PRESS_SEQ`,
...
//
// A SCHEDULE, not just a state: `CW_AUTOCHUCK="ITEM PICKER@0,ZOMBIE KILLER@180"` roams
...
{
    static const char* want = getenv("CW_AUTOCHUCK");
    ...
    if (!parsed)
    {
        ...
        while (at <= all.size())
        {
            ...
            if (state == -2)
            {
                fprintf(stderr, "[debug] CW_AUTOCHUCK: \"%s\" is not a state. Use OFF, 0..6, "
        ...
        fprintf(stderr, "[debug] CW_AUTOCHUCK schedule:");
    ...
    if (epoch < 0)
    {
        ...
        fprintf(stderr, "[debug] CW_AUTOCHUCK: %s engaged at %llds (object %08X, after %u "
    ...
    // We never write 4. So `CW_AUTOCHUCK` was only ever setting the INITIAL state and the
    ...
    // them apart. `CW_AUTOCHUCK_NO_HOLD=1` restores set-once, which is the control arm.
    static const bool noHold = getenv("CW_AUTOCHUCK_NO_HOLD") != nullptr;
    ...
    if (!noHold && appliedStep >= 0 && schedule[size_t(appliedStep)].state >= 0 && objectNow)
    {
        ...
        if (PPC_LOAD_U32(objectNow + 0x70) != desired)
        {
            ...
            {
                lastOverrideLog = now;
                fprintf(stderr, "[debug] CW_AUTOCHUCK: the title's AI has changed the state "
                                "away from %s %llu time(s); re-asserting (CW_AUTOCHUCK_NO_HOLD=1 "
                                "to let it win)\n",
    ...
    fprintf(stderr, "[debug] CW_AUTOCHUCK: step %d -> %s at %llds (%llds into the level)\n",
            want_step, st.state < 0 ? "OFF" : kAutoChuckStates[st.state],
            DebugElapsedSeconds(), since);
}

// CW_DEBUG_FLAGS=NAME[,NAME...] — set the title's own gameplay debug bools without the
...
// `CW_AUTOCHUCK` is: the bytes live in the guest image but the game writes them itself —
...
{
    static const char* spec = getenv("CW_DEBUG_FLAGS");
    if (!spec)
        return;
    ...
    // CW_DEBUG_TUNABLES does — a name that silently does nothing is indistinguishable
    ...
    static std::vector<const CustomBool*> wanted = [] {
        ...
        for (size_t pos = 0; pos <= s.size();)
        {
            ...
            {
                out.push_back(hit);
                fprintf(stderr, "[debug] CW_DEBUG_FLAGS: '%s' @%08X will be held ON\n",
                        hit->label, hit->address);
            }
            else
            {
                fprintf(stderr, "[debug] CW_DEBUG_FLAGS: unknown flag '%s'. Known:\n",
                        item.c_str());
                for (const CustomBool& b : kCustomBools)
    ...
    for (const CustomBool* b : wanted)
        if (!PPC_LOAD_U8(b->address))
        {
            ...
            if (++reasserts <= std::size(kCustomBools) || (reasserts % 1000) == 0)
                fprintf(stderr, "[debug] CW_DEBUG_FLAGS: '%s' set ON (%llu asserts so "
                                "far — anything above one means the title cleared it)\n",
                        b->label, (unsigned long long)reasserts);
        }
}

// CW_GUEST_DIAG=1 — the engine's ENTIRE diagnostic layer, switched back on by one byte.
//
// WHY THIS EXISTS
// ---------------
// `CW_GUEST_LOG` (runtime/cpu/guest_probe.cpp) hooks the engine's log sink and has
...
// CW_DEBUG_FLAGS is, and for the counter (gotcha 151).
//
// WHY THE SECOND BYTE
// -------------------
...
// CW_GUEST_LOG already reads. Silencing the trap is what makes the message reachable.
//
// WHAT IT IS FOR
// --------------
...
// four are behind this byte. Pair it with CW_GUEST_LOG=1, which is the sink — this arm
...
{
    static const bool on = getenv("CW_GUEST_DIAG") != nullptr;
    if (!on)
        return;
    ...
    if (!announced)
    {
        announced = true;
        fprintf(stderr,
                "[debug] CW_GUEST_DIAG: clearing the release log gate at %08X (was %u; "
                "2,013 call sites read it, none write it) and setting %08X so asserts "
                "print instead of trapping. Needs CW_GUEST_LOG=1 to see anything — this "
    ...
    if (reasserts != reported && (reasserts <= 2 || (reasserts % 1000) == 0))
    {
        reported = reasserts;
        fprintf(stderr,
                "[debug] CW_GUEST_DIAG: %llu writes so far (2 = set once as expected; "
...
static uint32_t LookupPlayerObject(PPCContext& ctx, uint8_t* base, uint32_t index)
{
    ...
    const bool trace = getenv("CW_POSE_TRACE") != nullptr;
...
// list. Armed by CW_POSE_TRACE so it costs nothing in an ordinary run.
static void CensusCallingThread(PPCContext& ctx, uint8_t* base)
{
    if (!getenv("CW_POSE_TRACE"))
        return;
                           ...
                           float x, float y, float z)
{
    ...
    // CW_TELEPORT_DRYRUN=1 stops here: everything except the two calls. It is how the
    // marshalling above can be checked on a run that is guaranteed not to crash.
    if (getenv("CW_TELEPORT_DRYRUN"))
    {
    ...
    static const bool callMode = [] {
        const char* m = getenv("CW_TELEPORT_MODE");
    ...
    if (!callMode)
    {
        ...
        fprintf(stderr, "[debug] teleport FIELD MODE: wrote +0x1C, +0x250, +0x620 and "
                        ...
                        "subsystem was notified (CW_TELEPORT_MODE=call for the real "
...
// CW_TELEPORT_FILE=<path> — one line, `x y z` (optionally `x y z player`), applied when
...
{
    static const char* path = getenv("CW_TELEPORT_FILE");
    if (!path)
        return;
    ...
    {
        announced = true;
        fprintf(stderr, "[debug] CW_TELEPORT_FILE armed: write `x y z` into %s to move "
    ...
    if (got < 3)
    {
        fprintf(stderr, "[debug] CW_TELEPORT_FILE: '%s' does not read as `x y z` — "
...
static void PumpPlayerPosCache(PPCContext& ctx, uint8_t* base)
{
    ...
    if (!getenv("CW_CAPTURE_KEY") && !getenv("CW_POSE_TRACE"))
        return;
    float p[3];
...
void DebugTunables_PumpDebugMenu(PPCContext& ctx, uint8_t* base)
{
        ...
        node < kCustomBoolBase + std::size(kCustomBools))
    {
        if (action == 1 || action == 2 || action == -1)
        {
            ...
            fprintf(stderr, "[debug] Case West bool '%s' @%08X -> %s\n",
...
// The set CW_DEBUG_MENU=1 turns on: the dev-only master gate, the menu/jump leaf
...
void ApplyFromEnvironment(uint8_t* base)
{
    ...
    // CW_DEBUG_TUNABLES=name[=0|1],name[=0|1],...  -- anything in the table above.
    if (const char* list = getenv("CW_DEBUG_TUNABLES"))
    {
        ...
        while (pos <= spec.size())
        {
            ...
            else
            {
                ...
                fprintf(stderr, "[debug] CW_DEBUG_TUNABLES: unknown name '%s'. "
                                "Known names:\n", item.c_str());
...
PPC_FUNC(sub_827F6D40)
{
    ...
    if (g_autoChuckHeld.load(std::memory_order_acquire))
    {
        static const std::vector<uint32_t> closeHashes = [] {
            ...
            const char* e = getenv("CW_AUTOCHUCK_CLOSE_HASHES");
            if (!e)
            ...
            now - g_autoBackLastMs >= 3)   // one press per screen, not per transition
        {
            ...
            fprintf(stderr, "[debug] AutoChuck opened screen %08X at %llds — pressing B in "
                            "%lldms to close it (%llu so far; CW_AUTOCHUCK_CLOSE_HASHES= to "
                            "disable, CW_AUTOCHUCK_CLOSE_DELAY_MS=N to retime)\n",
...
PPC_FUNC(sub_82189B00)
{
    ...
    const bool capture = getenv("CW_ZOMBIE_CAPTURE") != nullptr;
...
{
    if (ctx.r3.u32 == g_debugMenuObject && getenv("CW_DEBUG_MENU"))
    {
...
// pointer `CW_AUTOCHUCK` drives the AI through (`g_gameDebugController + 0x30`), which
...
extern "C" uint32_t CW_DebugPlayerObject()
{
...
{
    uint8_t* base = g_memory.base;
    const uint32_t obj = CW_DebugPlayerObject();
