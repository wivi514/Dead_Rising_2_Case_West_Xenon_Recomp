...
//
// Everything is inert unless CW_D3D_DRAW=1.
#include "d3d_draw.h"
...
void StoreGpuRaw(uint8_t* base, uint32_t physAddr, uint32_t value)
{
    ...
    // The walker's half of CW_PM4_MEM_WATCH (see pm4.cpp): with both halves
...
// is the same six halfwords, and CW_PM4_NO_SCREEN_EXTENT gates both.
...
uint32_t ExecutePacket(PPCContext& ctx, uint8_t* base, uint32_t va, uint32_t availDwords)
{
    ...
    switch (opcode)
    {
        ...
        case 0x36: // DRAW_INDX_2: dword0 = initiator
        {
            ...
            if (d.indexed && bodyCount > initiatorAt + 2)
            {
                ...
                static const bool oldReading =
                    getenv("CW_PM4_INDEX_ADDR_SWIZZLE") != nullptr;
        ...
        // rule as the index swizzle (CW_PM4_INDEX_ADDR_SWIZZLE): a decode the two
...
bool Init()
{
    if (!getenv("CW_D3D_DRAW") || *getenv("CW_D3D_DRAW") == '0')
    {
        g_mode = Mode::Off;
        return false;
    }
    if (getenv("CW_VKDRAW"))
    {
        ...
        fprintf(stderr, "[d3ddraw] CW_D3D_DRAW and CW_VKDRAW are mutually exclusive; "
                        "CW_D3D_DRAW DISABLED for this run\n");
        g_mode = Mode::Failed;
        return false;
    }
...
void D3dDraw_OnSwap(uint8_t* base)
{
    ...
    // The walker's counters ride the same cadence as the renderer's (CW_VK_STATS=N),
    // so one env var reports both halves of the phase C pipeline.
    static const long every = [] {
        const char* v = getenv("CW_VK_STATS");
...
bool D3dDraw_ServiceReserve(PPCContext& ctx, uint8_t* base)
{
    ...
    static const bool noKick = [] {
        const char* v = getenv("CW_D3D_NO_RESERVE_KICK");
        const bool on = v && *v && *v != '0';
        if (on)
            fprintf(stderr, "[d3ddraw] CW_D3D_NO_RESERVE_KICK=1 — the reserve is serviced "
    ...
    // assertion: CW_D3D_NO_ARM_SEG_DIRECT=1 restores the append.
    static const bool noDirect = [] {
        const char* v = getenv("CW_D3D_NO_ARM_SEG_DIRECT");
        const bool on = v && *v && *v != '0';
        if (on)
            fprintf(stderr, "[d3ddraw] CW_D3D_NO_ARM_SEG_DIRECT=1 — the arm-carrying "
