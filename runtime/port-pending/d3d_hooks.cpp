...
//
// Everything is behind CW_D3D_OBSERVE, read once; off, each hook costs one
...
// REPLACE mode (CW_D3D=1, phase B) services a subset of the hooks instead of
...
#define CW_D3D_HOOKS(X)                                                              \
    ...
    /* hook (CW_JOBQ_PROBE), and two strong definitions cannot link.               */\
...
bool Observe()
{
    static const bool on = [] {
        const char* v = std::getenv("CW_D3D_OBSERVE");
        bool en = v && *v && *v != '0';
        if (en)
...
{
    static const bool on = [] {
        const char* v = std::getenv("CW_D3D_DRAW");
        return v && *v && *v != '0';
    }();
    return on;
}
...
{
    static const bool on = [] {
        const char* v = std::getenv("CW_D3D");
        bool en = v && *v && *v != '0';
        if (DrawMode())
...
// ---------------------------------------------------------------------------------
// THE BIN-MASK PROBE — CW_BINMASK_PROBE=1
//
...
// (CW_PM4_BIN_CENSUS) localised that to one number: at the right tile's draws
...
ServiceResult Service(HookId id, PPCContext& ctx, uint8_t* base)
{
    ...
    case kH_PreSwapResolve:
        ...
        //
        // Same-binary control arm, same discipline as CW_D3D_NO_RESERVE_KICK: with
        // CW_D3D_REDIRECT_PRESWAP=1 the call goes back into the Redirect group, so
        ...
        // variable rather than one rebuild.
        {
            static const bool preFix = [] {
                const char* v = getenv("CW_D3D_REDIRECT_PRESWAP");
                const bool on = v && *v && *v != '0';
                if (on)
                    fprintf(stderr, "[d3d] CW_D3D_REDIRECT_PRESWAP=1 — sub_82841AD0's "
...
#define X(addr, name)                                                                \
    ...
    {                                                                                \
        ...
        /* decide? Behind CW_BINMASK_PROBE, and only on this one row.                */\
