...
// CW_NO_AUDIO_OUT=1 keeps the whole pipeline — pump, decode, mix — and opens no
...
void EnsureDevice()
{
    std::call_once(g_initOnce, [] {
        g_inited = true;
        if (getenv("CW_NO_AUDIO_OUT"))
        {
            g_disabled = true;
            fprintf(stderr, "[audio] output disabled (CW_NO_AUDIO_OUT) — frames are "
                            "mixed and dropped\n");
            return;
        }
#ifndef CW_HAVE_SDL
