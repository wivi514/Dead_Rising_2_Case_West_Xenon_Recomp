...
// The enumerator kinds the XDK wrappers pass in r4. Case West links only the
...
void XliveStats_Start()
{
    if (g_enabled)
        return;
    const char* on = std::getenv("CW_XLIVE_ONLINE");
    if (!on || on[0] != '1')
        return;
    g_enabled = true;
                                     ...
                                     be<uint32_t>* sizeOut, be<uint32_t>* handleOut)
{
    ...
    case kEnumByXuid:
        ...
        if (pivot == CwXlive_Xuid(0) && pivot != 0)
        {
...
// guest memory with no server. CW_XLIVE_STATS_TEST=1.
...
// The wrapper's formula, against the numbers Case West's own leaderboard
...
#define XLIVE_EXPECT_ONLINE(cond)                                                \
    ...
    // --- by name, the way sub_825947C8 does it ------------------------------
    {
        ...
        GuestScratch overlapped(28);
        *GuestPtr<be<uint64_t>>(xuids.va) = CwXlive_Xuid(0);
        auto* guestMsg = GuestPtr<GuestReadStats>(msg.va);
        ...
        if (status == kErrorSuccess)
        {
            ...
            const auto* rows = views ? GuestPtr<GuestStatsRow>(views[0].rowsPtr.get()) : nullptr;
            XLIVE_EXPECT_ONLINE(rows && rows[0].xuid.get() == CwXlive_Xuid(0));
            if (rows)
...
{
    const char* on = std::getenv("CW_XLIVE_STATS_TEST");
    if (!on || on[0] != '1')
        return;
    if (!g_enabled)
    {
        fprintf(stderr, "[xlive] stats self-test skipped: CW_XLIVE_ONLINE is not set\n");
        return;
    }
    if (Live().online())
    {
