...
std::string VfsResolveExisting(const std::string& guestPath)
{
    ...
    static const bool overlayOff = getenv("CW_NO_PATCHED_ASSETS") != nullptr;
    ...
    // the input path: a pad player (CW_NO_NATIVE_KBM=1) keeps pad prompts, and
    // CW_NO_KB_PROMPTS=1 keeps them with the keyboard live too. Checked BEFORE
    ...
    static const bool kbPromptsOn = NativeKbm_Enabled() &&
                                    getenv("CW_NO_KB_PROMPTS") == nullptr;
        ...
        if (!gameRoot.empty() && direct.rfind(gameRoot + "/", 0) == 0)
        {
            ...
            if (fs::exists(patched, ec))
            {
                KLOG("VFS: '%s' served from the KB-PROMPT overlay -> %s "
                     "(CW_NO_KB_PROMPTS=1 restores the pad glyphs)\n",
    ...
    // CW_NO_PATCHED_ASSETS=1 is the control arm: the shipped data, byte for byte.
    if (!overlayOff)
    {
        std::string gameRoot;
        {
        ...
        if (!gameRoot.empty() && direct.rfind(gameRoot + "/", 0) == 0)
        {
            ...
            if (fs::exists(patched, ec))
            {
                KLOG("VFS: '%s' served from the PATCHED overlay -> %s "
                     "(CW_NO_PATCHED_ASSETS=1 restores the shipped file)\n",
                     guestPath.c_str(), patched.c_str());
                std::lock_guard lock(g_mutex);
                g_resolved.emplace(guestPath, patched);
                return patched;
