...
// business. And it has more than one: Case West's host LISTENS on one port,
...
// and inet_addr. On real guest memory, signed out. CW_XLIVE_NET_TEST=1.
...
{
    const char* on = std::getenv("CW_XLIVE_NET_TEST");
    if (!on || on[0] != '1')
        return;
    if (!XliveNet_Enabled())
    {
        fprintf(stderr, "[xlive] net self-test skipped: CW_XLIVE_COOP is not set\n");
        return;
    }
    if (Live().online())
    {
