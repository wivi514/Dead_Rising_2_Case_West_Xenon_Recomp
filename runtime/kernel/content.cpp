...
void ContentSetRootFromGameDir(const std::string& gameDir)
{
    ...
    // without moving files again. CW_SAVE_DIR still overrides the whole root.
    const std::filesystem::path oldRoot =
        std::filesystem::path(gameDir).parent_path() / "save";
    if (const char* env = getenv("CW_SAVE_DIR"))
        g_saveRoot = env;
    else
    ...
    if (g_saveRoot != oldRoot && std::filesystem::is_directory(oldRoot, ec))
    {
        ...
        if (!newHasContainers)
        {
            ...
            std::error_code sec;
            if (std::filesystem::is_regular_file(oldRoot / "cw_settings.txt", sec))
                std::filesystem::copy_file(oldRoot / "cw_settings.txt",
                                           ContentSettingsDir() / "cw_settings.txt",
...
{
    if (getenv("CW_SAVE_DIR"))
    {
        KLOG("content: profile '%s' — CW_SAVE_DIR overrides, saves stay in %s\n",
...
std::filesystem::path ContentSettingsDir()
{
    ...
    // Under a CW_SAVE_DIR override everything stays in the one overridden dir.
    if (getenv("CW_SAVE_DIR"))
        return g_saveRoot;
    std::error_code ec;
