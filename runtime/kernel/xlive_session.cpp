...
void XliveSession_Start()
{
    if (g_enabled)
        return;
    const char* on = std::getenv("CW_XLIVE_COOP");
    if (!on || on[0] != '1')
        return;
    g_enabled = true;
...
void TestSearchResultBuffer()
{
    ...
    // A host that advertised what Case West's lobby advertises: two contexts
...
{
    const char* on = std::getenv("CW_XLIVE_COOP_TEST");
    if (!on || on[0] != '1')
        return;
