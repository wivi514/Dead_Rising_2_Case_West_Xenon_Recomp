...
// CW_FILE_RACY=1 — skip the per-handle IO lock: the pre-part-35 file layer, and the
...
static bool FileRacy()
{
    static const bool racy = [] {
        const char* v = getenv("CW_FILE_RACY");
        return v && *v && *v != '0';
    }();
    return racy;
}
...
static void NoteFileIoOverlap(FileHandle* file)
{
    ...
    if (n == 1 || (n & 1023) == 0)
        fprintf(stderr, "[file] positional-IO OVERLAP #%llu on '%s'%s\n",
                ...
                FileRacy() ? " (CW_FILE_RACY=1: NOT serialized)" : " (serialized)");
}
...
bool FileTrace()
{
    static const bool on = getenv("CW_FILE_TRACE") != nullptr;
    return on;
}
...
// CW_FILE_WRITE_SELFTEST=1 — drive the create/write/read/verify round trip through the
...
{
    if (!getenv("CW_FILE_WRITE_SELFTEST"))
        return;

    const fs::path dir = fs::temp_directory_path() / "cw_file_selftest";
    std::error_code ec;
