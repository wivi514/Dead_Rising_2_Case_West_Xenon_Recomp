...
void CwXlive_Start(uint32_t titleId)
{
    ...
    if (const char* off = std::getenv("CW_NO_XLIVE"); off && off[0] == '1')
    {
        KLOG("[xlive] disabled by CW_NO_XLIVE\n");
        return;
    }
    if (titleId == 0)
    {
...
bool CwXlive_SignedIn()
{
    ...
    {
        // Back online — clear any pending drop so the next blip gets a fresh grace,
    ...
    // Offline. SELF-ARM the grace on the FIRST offline read (part 12). The gateway
    // flaps every few minutes and recovers in ~2 s, but libxlive logs those as
    // "gateway closed: connection ended" WITHOUT firing the ConnectionChanged event
    // that used to arm this hold — so g_dropSinceMs stayed -1, this returned false the
    // instant online() dipped, and the title's periodic check read a sign-out and tore
    // the co-op session down ("HW MM session found account: 0 is not signed in to xbox
    // live!" -> DELETING_SESSION, captured on a two-machine session). Arming here, on
    // the observed offline itself, holds the title's sign-in over any blip wherever it
    // came from; a real outage past the grace still tells the title, once.
    long long since = g_dropSinceMs.load();
    if (since < 0)
    {
        if (SigninGraceMs() <= 0)
            return false;   // grace disabled (CW_XLIVE_SIGNIN_GRACE_MS=0): the control arm
        g_dropSinceMs.store(NowMs());
        g_dropAnnounced.store(false);
