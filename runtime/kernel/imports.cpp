...
// `ppc/ppc_recomp_shared.h`'s 247 `__imp__` externs (gotcha 10 — the image is the
// authority, not the previous port and not the capture). A1's own import dump
// agrees: 247 `F` entries and 13 `V` entries. `tools/gen_import_stubs.py` scans
// this file and refuses to emit a stub for anything defined here, so the two never
// collide.
//
// Where Asura's Wrath's runtime had an implementation for an import this image does
// not declare, it is simply absent rather than carried along — Carrying those would be
// dead code that reads like coverage.
//
// **Case West restores the MUTANT PAIR**, which Case Zero dropped under exactly that
// rule and which this title does import. It is not a formality: a solo gameplay session
// calls `NtReleaseMutant` 32,382 times (finding 3). `DbgBreakPoint` is restored too and
// is genuinely never called — but it logs rather than doing nothing, because a guest
// asking to break is the most informative event a run can produce.
//
// ORDER OF WORK
// -------------
...
#include "xlive_session.h" // XenonLive: this machine's XNADDR for XNetGetTitleXnAddr
...
// CW_MEM_TRACE=1 — every virtual-memory call, with its arguments AND its answer.
//
...
{
    static const bool on = getenv("CW_MEM_TRACE") != nullptr;
    return on;
}
...
// CW_CS_STATS=1: how many acquisitions actually reach each phase. This is the
...
static void RtlEnterCriticalSection_x(XRTL_CRITICAL_SECTION* cs)
{
    ...
    static const bool csTrace = getenv("CW_CS_TRACE") != nullptr;
    static const bool csStats = getenv("CW_CS_STATS") != nullptr;
    // CW_CS_NO_BACKOFF=1 restores the pure yield spin. It is the same-binary control
    ...
    static const bool noBackoff = getenv("CW_CS_NO_BACKOFF") != nullptr;
    ...
    for (uint32_t attempt = 0;; attempt++)
    {
        ...
        // CW_CS_TRACE=1: name the owner of a section this thread cannot get. A frozen
...
// A RECURSIVE mutant, and the only kernel object Case West needs that Case Zero does
// not. Case Zero's runtime deliberately dropped the pair as dead code (see this file's
// header); it is restored here from the Asura's Wrath implementation, which is the one
// port of the three that had it.
//
// WHY IT MUST BE REAL, not a stub that returns STATUS_SUCCESS
// -----------------------------------------------------------
// Capture analysis finding 3: a *solo* 5-10 minute gameplay session calls
// `NtCreateMutant` 6 times and `NtReleaseMutant` **32,382** times. Six mutants released
// ~32k times is a per-frame or per-resource lock on the ordinary single-player path —
// the volume points at audio/XMA or the streaming loader. The port's earlier guess that
// the pair "probably belongs to co-op" was refuted by that measurement, so this is not
// an optional subsystem behind a feature we are not building.
//
// A mutant that fakes acquisition is gotcha 5's failure mode with a deadlock or a data
// race at the end of it, on the hot path, where it will be blamed on anything else.
//
// Ownership is per guest thread and RECURSIVE: the same thread may acquire repeatedly
// and only the last release frees it. `Release` from a thread that does not own it is
// STATUS_UNSUCCESSFUL rather than a silent success, because a guest that double-releases
// has a bug we want to see rather than paper over.
struct Mutant final : KernelObject
{
    ...
    uint32_t Wait(uint32_t timeoutMs) override
    {
        const uint32_t self = CurrentGuestThreadId();
        std::unique_lock lock(m);
        auto ready = [&] { return owner == 0 || owner == self; };
        if (timeoutMs == WAIT_TIMEOUT_INFINITE)
            cv.wait(lock, ready);
        else if (!cv.wait_for(lock, std::chrono::milliseconds(timeoutMs), ready))
            return STATUS_TIMEOUT;
    ...
    {
        std::lock_guard lock(m);
        if (recursion == 0 || owner != CurrentGuestThreadId())
...
// The mutant pair — Case West only. See the `Mutant` struct above for why these are
// real rather than stubs (32,382 releases in one solo gameplay session).
//
// Signature note: `initialOwner` is the third argument on 360 as on Win32
// (`NtCreateMutant(OUT handle, IN attrs, IN initialOwner)`), and the guest is entitled
// to pass a non-zero value, so it is honoured rather than assumed zero. The
// out-parameter is filled on every failure path, because a guest that ignores the
// status and reads the handle anyway is gotcha 5's exact failure mode.
static uint32_t NtCreateMutant_x(be<uint32_t>* handle, XOBJECT_ATTRIBUTES* attrs,
                                 uint32_t initialOwner)
{
    if (!handle)
        return STATUS_INVALID_PARAMETER;
    Mutant* mutant = CreateKernelObject<Mutant>(initialOwner != 0);
    if (!mutant)
    {
        *handle = 0;
        return STATUS_NO_MEMORY;
...
// `previousCount` is the recursion count BEFORE this release, which is what the Win32
// contract says and what a caller balancing acquisitions would read. Filled on the
// failure path too.
static uint32_t NtReleaseMutant_x(Mutant* mutant, be<int32_t>* previousCount)
{
    if (!mutant)
    {
        if (previousCount)
    ...
    const int32_t previous = static_cast<int32_t>(mutant->recursion);
    const uint32_t status = mutant->Release();
    if (previousCount)
...
// Wait on a kernel object, and under CW_WAIT_TRACE=1 name it if it never returns.
//
...
{
    static const bool on = getenv("CW_WAIT_TRACE") != nullptr;
    return on;
}
                                          ...
                                          be<int64_t>* timeout)
{
    ...
    // CW_WAIT_TRACE=1 names this if it never returns; see WaitObject.
...
//  - CW_WAIT_TRACE names it. See ReportStuckMultiWait.
//
...
// Case West only — Case Zero does not import these. Finding 3.
...
// CW_STALL_TRACE=<n>: every n-th sleep on a thread, print that thread's identity and
...
static uint32_t StallTraceInterval()
{
    static const uint32_t interval = [] {
        const char* v = getenv("CW_STALL_TRACE");
...
{
    // Under CW_DETERMINISTIC_CLOCK this must follow the SAME virtual clock as `mftb`,
    ...
    if (cw_timebase::deterministic)
        return uint64_t((__uint128_t(cw_timebase::virtual_ticks) * 10'000'000ull) /
                        CW_TIMEBASE_HZ);
...
// DbgBreakPoint — Case West only (one of the three imports Case Zero lacks).
//
// Neither capture ever calls it: A1 (boot to title) and A2 (5-10 min of gameplay) both
// report zero, which is what you expect from a routine that fires only on an actual
// debug break. That makes it the one of the three that is genuinely safe to leave
// inert — but inert is NOT the same as silent.
//
// On hardware with no debugger attached this traps into the kernel and, in a retail
// context, does nothing useful. There is no return value to fake, so "fail honestly"
// here means: continue execution, and make sure the operator KNOWS the guest asked to
// break, with enough context to find out why. A guest reaching this has hit an
// assertion or an unhandled case, and a silent no-op would turn the most informative
// event in a run into nothing at all.
//
// It is not an abort: a debug break in a retail image is frequently non-fatal (the
// image still carries its debug build — see CLAUDE.md's game intel), and killing the
// process would lose everything after it.
PPC_FUNC(__imp__DbgBreakPoint)
{
    KCALL("DbgBreakPoint");
    static std::atomic<int> hits{ 0 };
    const int n = hits.fetch_add(1) + 1;
    if (n <= 16)
        fprintf(stderr,
                "[DbgBreakPoint] *** the guest requested a debug break *** #%d "
                "lr=%08X r3=%08X tid=%08X — continuing; neither capture ever reached "
                "this, so it is worth finding out what did\n",
                n, g_ppcContext ? uint32_t(g_ppcContext->lr) : 0, ctx.r3.u32,
                CurrentGuestThreadId());
    else if (n == 17)
        fprintf(stderr, "[DbgBreakPoint] (further breaks suppressed)\n");
}
...
// The title-requested exit, shared by the two imports below (imported from Case Zero
// aea4292, their part 99). The same sequence as window.cpp's Shutdown and main.cpp's
// signal handler, for the same reasons: dump the counters (an operator session's
// numbers must not vanish with the quit), write the pipeline cache back (the next
// launch's warmth depends on this firing on the NORMAL quit path), and _Exit rather
// than exit — other guest threads are still executing recompiled code against guest
// memory, and running static destructors underneath them turns an ordinary quit into
// a crash report. HalReturnToFirmware used plain exit(0) from phase 1 until now,
// which was that crash risk plus a silently discarded pipeline cache on any run that
// ended through it.
...
// The main menu's EXIT GAME -> YES path. An XBLA title leaves by relaunching the
// dashboard — XamLoaderLaunchTitle(NULL, flags) — which our generated stub answered
// with STATUS_NOT_IMPLEMENTED, so confirming the exit did nothing and the player had
// to close the window. On a single-title runtime every launch target means the same
...
static uint32_t XexGetProcedureAddress_x(uint32_t module, uint32_t ordinal, be<uint32_t>* out)
{
    ...
    if (!known)
    {
        ...
        // for refusing to mint one blind). `CW_KCALL_WHO` cannot reach this, because it
...
// silently when they differ. The first two-machine co-op session hung on
...
// message: XSessionCreate (sub_82605E28) calls XamSessionCreateHandle(&h) and
...
// the handle when it is done (sub_825AAC78, CloseHandle), which is when the
// object goes.
...
//
// The pad is still reported connected when the runtime is headless (CW_NO_WINDOW=1
// or -DCW_WINDOW=OFF). That is deliberate and it is the honest reading of the
                                   ...
                                   GuestInputState* state)
{
    ...
    static const int fakeStartMs = []() {
        ...
        if (ms > 0)
            KLOG("CW_FAKE_START_MS=%d — SYNTHETIC INPUT IS ON. This run's progress is "
    ...
    // CW_FAKE_PRESS_SEQ=START,A,A — the SEQUENCE the arm walks, one button per
    ...
    // levels past the title and therefore past everything CW_FAKE_START_MS can reach.
    ...
    // window and the whole recipe runs headless. Requires CW_DEBUG_MENU=1; without it the
    ...
    static const std::vector<NamedButton> sequence = [] {
        ...
        const char* e = getenv("CW_FAKE_PRESS_SEQ");
        if (!e || !*e)
            return seq;
        ...
        for (size_t i = 0; i <= all.size(); i++)
        {
            if (i == all.size() || all[i] == ',')
            {
                ...
                {
                    unknown++;
                    KLOG("CW_FAKE_PRESS_SEQ: UNKNOWN entry \"%s\" IGNORED — the rest of "
        ...
        KLOG("CW_FAKE_PRESS_SEQ: %zu synthetic inputs queued, one per interval (%zu "
    ...
    //
    // CW_FAKE_PRESS_EDGE_MISS=1 — THE POSITIVE CONTROL FOR THE RECOVERY ABOVE, and the
    ...
    static const bool edgeMiss = getenv("CW_FAKE_PRESS_EDGE_MISS") != nullptr;
    if ((started || parked) && !(edgeMiss && active && entry.hostKey))
    {
        ...
        for (size_t j = 0; j <= idx && j < sequence.size(); ++j)
        {
            ...
            // not installed — CW_DEBUG_MENU unset — the press is consumed and nothing
            ...
            KLOG("CW_FAKE_PRESS_SEQ: synthetic %s host debug edge at %llds (interval %zu"
                 "%s). Needs CW_DEBUG_MENU=1 to have any effect.\n", sequence[j].name,
    ...
    if (stateKey != lastState.exchange(stateKey))
    {
        ...
        if (lerping && lastLoggedIdx.exchange(idx) == idx)
            ...
            KLOG("CW_FAKE_START_MS: synthetic %s %s at %llds (packet %u)\n", entry.name,
    ...
    if (outButtons & 0x0020)
    {
        ...
        if (backs++ == 0)
            KLOG("CW_FAKE_PRESS_SEQ: BACK delivered to the guest at %llds (entry \"%s\") — "
...
PPC_FUNC(__imp__XamInputGetState)
{
    ...
    // CW_AUTOCHUCK=<state> — hands Chuck to the AI as soon as a level exists, replacing
