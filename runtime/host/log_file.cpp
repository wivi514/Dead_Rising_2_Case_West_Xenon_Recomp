...
//
// Why: a shipped v1.0.2 session wrote a 98 GB cw_runtime.log and a 68 GB .log.1 beside
...
//
// CW_LOG_MAX_MB overrides it; 0 means no cap, for a deliberate long capture.
constexpr unsigned long long kDefaultLogCapBytes = 256ull * 1024 * 1024;

unsigned long long LogCapBytes()
{
    if (const char* m = getenv("CW_LOG_MAX_MB"))
    {
        ...
        if (g_file >= 0 && !capped)
        {
            ...
            if (cap && toFile >= cap)
            {
                ...
                const int k = snprintf(note, sizeof note,
                        ...
                        "reporting. CW_LOG_MAX_MB=N raises it, CW_LOG_MAX_MB=0 removes it.\n",
                        cap / (1024ull * 1024ull));
                if (k > 0)
...
bool Begin(const std::filesystem::path& dir, const char* name)
{
    if (g_live)
        return true;
    if (const char* off = std::getenv("CW_NO_LOG_FILE"); off && *off && std::strcmp(off, "0") != 0)
    {
        std::fprintf(stderr, "[log] CW_NO_LOG_FILE is set — no log file this run "
                             "(stderr only)\n");
        return false;
    }
    ...
    int fd = -1;
    if (const char* env = std::getenv("CW_LOG_FILE"); env && *env)
    {
        chosen = env;
        Rotate(chosen);
        fd = OpenLog(chosen);
        if (fd < 0)
            std::fprintf(stderr, "[log] CW_LOG_FILE=%s cannot be opened (%s) — trying the "
