...
//         dev      <repo>/runtime/build/cw_runtime   -> <repo>          (2 levels up)
//         release  CaseWestRecomp/cw_runtime         -> CaseWestRecomp  (0 levels up)
...
//   Windows  <FOLDERID_SavedGames>\Dead Rising 2 Case West   (the real Saved Games
//            known folder — localized, redirection-aware; falls back to
//            %USERPROFILE%\Saved Games if the shell cannot answer)
//   Linux    $XDG_DATA_HOME (default ~/.local/share)/Dead Rising 2 Case West
//   macOS    ~/Library/Application Support/Dead Rising 2 Case West
// CW_SAVE_DIR (read in content.cpp) still overrides the whole save root for tests.
