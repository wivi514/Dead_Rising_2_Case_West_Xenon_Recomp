...
// the platform's XenonLive folder), so game and launcher agree; CW_BUG_REPORT_DIR
// overrides it. Nothing here touches the network — the launcher shows the capture,
// the player decides.
//
// The dev-side instruments on the same keys (CW_CAPTURE_KEY's census and snapshots,
// CW_BURST_DUMP's every-frame burst) are untouched and still fire when armed; this
// is the always-on half a shipped build carries. CW_BUG_REPORTS=0 turns it off.
//
// The folder is bounded: CW_BUG_REPORT_MAX_MB (default 256) and at most 40 captures;
