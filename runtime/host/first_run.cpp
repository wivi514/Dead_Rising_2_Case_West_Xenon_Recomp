...
std::string Explain(Status s, const std::string& xexPath)
{
    ...
    case Status::NoPackage:
    {
        ...
        std::string m =
            "No Dead Rising 2: Case West package found.\n"
            "\n"
    ...
    case Status::NoShaderCache:
        return "No translated shader cache.\n"
               ...
               "\n"
               "  Or run without the renderer (CW_VKDRAW unset), which needs no cache.\n";
    }
    return {};
}
...
{
    if (const char* e = std::getenv("CW_NO_FIRST_RUN_CHECK"); e && *e && *e != '0')
    {
        std::fprintf(stderr, "[firstrun] CW_NO_FIRST_RUN_CHECK=%s — checks skipped.\n", e);
        return true;
    }
    ...
    if (s == Status::NoShaderCache && !renderer)
    {
        std::fprintf(stderr, "[firstrun] no shader cache, but the renderer is off "
                             "(CW_VKDRAW unset) — proceeding.\n");
        return true;
    }
    if (s == Status::Ready)
        return true;

    std::fprintf(stderr, "\n=== Dead Rising 2: Case West — cannot start ===\n\n%s\n",
