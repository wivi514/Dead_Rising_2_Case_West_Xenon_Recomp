...
// The synthetic presses (CW_FAKE_PRESS_SEQ F9/F8) are the same key to the bug-report
...
HostPadState ReadKeyboard()
{
    ...
    if (g_keyboardFocus)
    {
        ...
        f7WasDown = f7Down;
        // F8/F9: the dev instruments (CW_BURST_DUMP, CW_CAPTURE_KEY) when armed, AND
...
void Host_WindowRun()
{
    ...
    for (;;)
    {
        ...
        // Text input follows the overlay's text boxes; off otherwise (above).
