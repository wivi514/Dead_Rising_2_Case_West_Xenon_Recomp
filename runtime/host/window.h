...
//
// Returns false when the runtime is deliberately headless (CW_NO_WINDOW=1) or when
...
// no renderer — CW_VKDRAW off is the control arm for every claim phase 5 makes, and in
...
// The same three edges, requested WITHOUT a keyboard — what `CW_FAKE_PRESS_SEQ`'s F2/F3/F4
...
// window; that is why these live outside the CW_HAVE_SDL split rather than beside the
...
//
// The fixed `CW_VK_SNAP_FRAME` trigger it joins is right for a boot-time question and
