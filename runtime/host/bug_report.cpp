...
#ifndef CW_GAME_VERSION
#define CW_GAME_VERSION "dev"
#endif
...
uint64_t MaxBytes()
{
    if (const char* m = getenv("CW_BUG_REPORT_MAX_MB"))
...
void WriteReport(std::unique_ptr<Pending> p)
{
    ...
    std::string sys;
    {
        char b[1024];
        snprintf(b, sizeof b,
                 "game: Dead Rising 2: Case West (recomp) %s\n"
                 "os: %s\n"
                 "cpu: %s (%u threads)\n"
                 "gpu: %s\n"
                 "driver: %s\n"
                 ...
                 "signed_in: %s\n"
                 "trigger: %s at %s\n",
                 CW_GAME_VERSION, os.c_str(), cpu.c_str(), std::thread::hardware_concurrency(),
                 ...
                 Settings_Fov(), CwXlive_SignedIn() ? "yes" : "no", p->trigger.c_str(),
    ...
    }
    {
        ...
        snprintf(b, sizeof b,
                 ...
                 DisplayModeName(Settings_DisplayMode()), JsonEscape(CW_GAME_VERSION).c_str());
        j += b;
    }
...
void BugReport_Init()
{
    if (const char* e = getenv("CW_BUG_REPORTS"); e && *e == '0')
    {
        fprintf(stderr, "[bugreport] off (CW_BUG_REPORTS=0): F8/F9 write no capture for the launcher\n");
        return;
    }
    std::string dir = getenv("CW_BUG_REPORT_DIR") ? getenv("CW_BUG_REPORT_DIR") : CwXlive_CapturesDir();
    g_dir = dir;
    std::error_code ec;
    ...
    fprintf(stderr, "[bugreport] F9 (one frame) / F8 (three frames) capture a bug report for the "
                    ...
                    "and 15 s after, the machine. CW_BUG_REPORTS=0 turns it off.\n",
            g_dir.string().c_str());
    Prune();
}
