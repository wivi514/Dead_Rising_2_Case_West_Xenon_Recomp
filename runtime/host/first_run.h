...
// `xexPath` is the image the run will actually load — an explicit argv[1] or CW_XEX
...
// Ready. `xexPath` is named in the message so that a run pointed elsewhere by CW_XEX
...
// renderer is going to be asked for — a headless gate run with CW_VKDRAW unset does
...
//
// CW_NO_FIRST_RUN_CHECK=1 skips the whole thing. It exists so that a session can
