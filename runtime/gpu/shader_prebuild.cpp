...
//   seed can build every pipeline at boot on session one. CW_NO_VS_RECIPES=1 is the
                  ...
                  const std::filesystem::path& recipes)
{
    ...
    if (const char* off = getenv("CW_NO_VS_RECIPES"); off && *off && strcmp(off, "0") != 0)
        fprintf(stderr, "[prebuild] CW_NO_VS_RECIPES=1 — pixel-only pass; the vertex half "
