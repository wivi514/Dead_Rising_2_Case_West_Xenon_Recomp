...
{
    if (!getenv("CW_ISR_TRACE") || !g_pumpIsr.userData)
        return;
...
// stalls the ring forever — which is exactly what a faithful CW_PM4_STOP_ON_WAIT run
...
// reporting each CPU in its PCR while it does. CW_ISR_SINGLE_CPU=1 is the same-binary
...
// arithmetic exactly, so `CW_FPS_CAP=30` still reproduces the shipped 16 ms pacing
// bit for bit.
//
...
int VblankPeriodUs()
{
    ...
    else if (const char* c = getenv("CW_FPS_CAP"))
    {
...
{
    ThreadBudget_NameSelf("cw-pump");
    GuestThreadContext threadContext(2);
    uint8_t* base = g_memory.base;

    // CW_VBLANK_MS: the interrupt cadence, 16 ms ~= the 360's 60 Hz vblank. An env
    ...
    // delta is sleep and none of it is GPU. **`CW_PM4_TICK_MS=16` is now the control
    // arm** for every claim above.
    const char* tickEnv = getenv("CW_PM4_TICK_MS");
    const int tickMs =
    ...
    //
    // CW_PM4_NO_CP_INTERRUPT=1 leaves it unregistered, so the ring is still consumed
    ...
    if (!getenv("CW_PM4_NO_CP_INTERRUPT"))
        Pm4_SetInterruptSink(DeliverCommandProcessorInterrupt);
    else
        KLOG("CW_PM4_NO_CP_INTERRUPT: ring will be consumed but source 1 stays down\n");
    ...
    //
    // Registering the draw sink only when Init succeeds is what keeps CW_VKDRAW a true
    ...
    if (VkRenderer_Init())
    {
        ...
        // Part 117: CW_PUMP_SPLIT=1 moves every renderer call off this thread onto
        // `cw-draw`, in stream order; this thread keeps the walk, the register file,
    ...
    // per hand-off.
    //
    // CW_PM4_NO_EAGER_TICK=1 restores the unconditional sleep (the control arm).
    // Forced off under CW_VBLANK_TICKCOUNT, whose legacy accounting defines a tick AS
    // one tickUs sleep — eager ticks would advance its vblank clock at yield speed.
    const bool eagerTick = getenv("CW_PM4_NO_EAGER_TICK") == nullptr &&
                           getenv("CW_VBLANK_TICKCOUNT") == nullptr;
    KLOG("eager tick %s\n",
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            split::Nap(napUs);   // a plain sleep unless CW_PUMP_SPLIT=1 (pump_split.h)
        }
        skipSleep = false;
        ...
        // for two reasons: with CW_PM4_TICK_MS unset this keeps the loop byte-for-byte
        ...
        if (userData && Pm4_RingInitialized())
        {
            ...
            // CW_PM4_NO_MIDWALK_RPTR=1 is the control arm: the slot stays 0 and the
            ...
            static const bool noMidwalk = [] {
                const bool off = getenv("CW_PM4_NO_MIDWALK_RPTR") != nullptr;
                KLOG("mid-walk rptr publication %s\n",
                     off ? "OFF (CW_PM4_NO_MIDWALK_RPTR — end-of-walk only)"
                         : "ON (guest sees ring consumption per packet; "
                           "CW_PM4_NO_MIDWALK_RPTR=1 restores end-of-walk only)");
                return off;
            }();
        ...
        // CW_PUMP_POISON_MS=N — THE POSITIVE CONTROL FOR THE SLOW-FRAME TABLE'S RESIDUAL
        ...
        // spike has to appear in the WORST-frame table by name.
        {
            static const int poisonMs = [] {
                const char* e = getenv("CW_PUMP_POISON_MS");
                const int v = e ? atoi(e) : 0;
                if (v > 0)
                    fprintf(stderr,
                            "[vd] CW_PUMP_POISON_MS=%d — burning %d ms every 64th pump "
        ...
        //
        // The vblank is scheduled on a DEADLINE. CW_VBLANK_TICKCOUNT=1 restores the
        ...
        static const bool wallClockVblank = getenv("CW_VBLANK_TICKCOUNT") == nullptr;
        if (wallClockVblank)
        {
        ...
        if (getenv("CW_RING_TRACE") && userData && (ticks % (1000 / vblankMs)) == 0)
        {
            const uint32_t kickedWptr = PPC_LOAD_U32(userData + kDeviceKickedWptr);
            ...
            // count (gotcha 109). It reads 0 unless CW_PM4_FENCE_MONOTONIC is on.
        ...
        //
        // CW_NO_VBLANK_GATE=1 is the same-binary control arm: the pre-fix runtime, in
        ...
        static const bool vblankGate = getenv("CW_NO_VBLANK_GATE") == nullptr;
        if (vblankGate)
        ...
        // is what clears it.
        if (getenv("CW_SWAPQ_TRACE") && userData && (ticks % (1000 / vblankMs)) == 0)
        {
...
// so with CW_FPS_CAP or CW_VBLANK_MS set this refuses loudly and changes nothing.
void Vd_SetFpsCapLive(int fps)
{
    VblankPeriodUs();   // resolve the initial value (and the locked flag) first
    if (g_vblankPeriodLocked)
    {
        fprintf(stderr, "[vd] frame cap menu change REFUSED — CW_FPS_CAP/CW_VBLANK_MS "
...
PPC_FUNC(__imp__VdSwap)
{
    ...
    //
    // CW_NO_SWAP_PAD=1 leaves the tail unfilled — the behaviour before finding 39, kept
    ...
    static const bool noPad = getenv("CW_NO_SWAP_PAD") != nullptr;
