...
// CW_PUMP_SPLIT_MB=N (a power of two, 2..64) sizes the ring — the arm for "does a ring
...
bool Start(uint8_t* base, void (*deliverInterrupt)())
{
    ...
    // below. CW_PUMP_SPLIT=1 forces it on anywhere, =0 forces the one-thread pump
    // anywhere; either spelling is the same-binary control arm.
    const char* e = getenv("CW_PUMP_SPLIT");
    ...
    {
        fprintf(stderr, "[split] CW_PUMP_SPLIT refused under CW_D3D_DRAW: the split is the "
                        "PM4 feed's; the API feed has its own thread\n");
        return false;
    }
    g_base = base;
    g_deliver = deliverInterrupt;
    if (const char* mb = getenv("CW_PUMP_SPLIT_MB"))
    {
        ...
        if (m >= 2 && m <= 64 && (m & (m - 1)) == 0)
        {
            ...
            fprintf(stderr, "[split] CW_PUMP_SPLIT_MB=%s ignored (a power of two, 2..64)\n", mb);
    }
    ...
    ThreadBudget_Note("draw", 1,
                      "CW_PUMP_SPLIT=1: the renderer on its own core; the pump thread keeps "
    ...
    fprintf(stderr, "[split] two-core pump %s (%u physical cores) — the PM4 walk stays on "
                    "cw-pump; draws, stores, swaps and interrupts execute in stream order "
                    "on cw-draw (one %u MB stream). CW_PUMP_SPLIT=0 is the one-thread "
                    "control\n",
            (e && *e) ? "by CW_PUMP_SPLIT=1" : "by default", physical,
