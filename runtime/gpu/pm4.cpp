...
// under CW_PM4_NO_MIDWALK_RPTR=1, which restores end-of-walk-only publication and is
...
//
// CW_PM4_NO_STOP_ON_WAIT=1 is the same-binary control arm for every claim above, and
// the flag CW_PM4_STOP_ON_WAIT still works so older recipes keep meaning what they
...
// ring (CW_ISR_SINGLE_CPU=1 with the brake on).
const bool g_stopOnWait = getenv("CW_PM4_NO_STOP_ON_WAIT") == nullptr;

// CW_PM4_ZERO_IS_NOP — the rejected reading of a zero header, kept as a measurable
// arm rather than deleted; see ExecutePacket.
const bool g_zeroIsNop = getenv("CW_PM4_ZERO_IS_NOP") != nullptr;
...
// SetGammaRamp (`sub_8284D860` in CASE ZERO's image — not re-derived here) emits, per entry, one type-0 write of
...
// walk (cw-pump) writes, the present (cw-draw, or the pump itself without the split)
...
// CW_PM4_BIND_CHECK=1 (player issue #3): at every draw, the pixel-shader binding is
...
const bool g_bindCheck = getenv("CW_PM4_BIND_CHECK") != nullptr;
...
// CW_SHADER_DUMP=<dir> — write one file per distinct microcode blob, named by the same
...
const char* g_shaderDumpDir = getenv("CW_SHADER_DUMP");
...
void DumpShader(uint32_t type, uint64_t hash, const uint8_t* code, uint32_t sizeDwords)
{
    ...
    // ten-minute recovery run to exactly that. `CW_VK_FRAME_DUMP` was fixed for this same
    ...
    else if (!complained)
    {
        complained = true;
        fprintf(stderr,
                "[imload] CW_SHADER_DUMP cannot write %s — NO microcode will be dumped "
                "this run (%s)\n",
                path, strerror(errno));
    }
}
...
//
// CW_PM4_STOP_ON_WAIT makes the command processor do what hardware does: hold at an
...
// CW_PM4_IB_VERIFY=1 — snapshot every indirect buffer before walking it and compare
// afterwards. See ExecuteLinearVerified for what question this answers and why it is
// not on by default.
const bool g_ibVerify = getenv("CW_PM4_IB_VERIFY") != nullptr;
...
const bool g_fenceMonotonic = [] {
    const char* v = getenv("CW_PM4_FENCE_MONOTONIC");
    const bool on = v && *v && *v != '0';
    if (on)
        fprintf(stderr, "[pm4] CW_PM4_FENCE_MONOTONIC=1 — GPU stores that move the fence "
...
bool StoreGpuRaw(uint8_t* base, uint32_t physAddr, uint32_t value)
{
    ...
    // CW_PM4_MEM_WATCH=<hex guest VA>: print every GPU store landing on that word,
    // with a monotonic timestamp. The memory sibling of CW_PM4_CONST_WATCH, added
    ...
    {
        static const char* w = getenv("CW_PM4_MEM_WATCH");
    ...
    // CW_PM4_FENCE_MONOTONIC=1 — an EXPERIMENT, not a fix, and it announces itself.
    //
    ...
    if (g_fenceMonotonic && va == g_fenceWord.load(std::memory_order_relaxed))
    {
        ...
        if (int32_t(value - current) < 0)
        {
            ...
            if (n <= 4 || (n & 0xFFFF) == 0)
                fprintf(stderr, "[pm4] CW_PM4_FENCE_MONOTONIC: REFUSED fence store "
...
//
// CW_PM4_NO_SCREEN_EXTENT=1 restores the pre-part-11 behaviour on BOTH streams, so
...
const bool g_noScreenExtent = [] {
    const char* v = getenv("CW_PM4_NO_SCREEN_EXTENT");
    return v && *v && *v != '0';
}();
...
// --- register writes --------------------------------------------------------------
// CW_PM4_CONST_WATCH=<hex>[-<hex>] — log writes to one register of the file, or to a
...
// could ever produce.
//
// CW_PM4_CONST_WATCH_FRAME=N holds the report until frame N, because the era that
// matters is never the boot (gotcha 139); CW_PM4_CONST_WATCH_ZEROS=1 restricts it to
// zero writes, which is what this instrument did when it only had one job.
const char* g_constWatchEnv = getenv("CW_PM4_CONST_WATCH");
...
{
    const char* e = getenv("CW_PM4_CONST_WATCH_FRAME");
    return e ? uint32_t(strtoul(e, nullptr, 10)) : 0u;
}();
const bool g_constWatchZerosOnly = getenv("CW_PM4_CONST_WATCH_ZEROS") != nullptr;
                       ...
                       int depth)
{
    ...
    //
    // CW_PM4_ZERO_IS_NOP=1 selects the rejected reading, so the question can be
    ...
    //
    // CW_PM4_NO_PREDICATION=1 executes predicated packets anyway — the same-binary arm
    ...
    static const bool noPredication = getenv("CW_PM4_NO_PREDICATION") != nullptr;
    const bool predicated = (header & 1) && (g_binMask & g_binSelect) == 0;

    // CW_PM4_BIN_TRACE=N — N lines of the predication story in stream ORDER: every
    ...
    if (binTraceEnv)
    {
        ...
        // CW_PM4_BIN_TRACE_FRAME=N holds it until swap N (player issue #3's era
        // begins a minute into a roam, past any line budget spent from boot).
        static const char* armFrameEnv = getenv("CW_PM4_BIN_TRACE_FRAME");
    ...
    // CW_PM4_LOADTRACE_EVERY=N: on every N-th swap, every shader load and every draw
    ...
    static const uint64_t loadTraceEvery = getenv("CW_PM4_LOADTRACE_EVERY")
        ? strtoull(getenv("CW_PM4_LOADTRACE_EVERY"), nullptr, 10) : 0;
    ...
    switch (opcode)
    {
        ...
        case 0x36: // DRAW_INDX_2: dword0 = VGT_DRAW_INITIATOR
        {
            ...
            if (d.indexed && bodyCount > initiatorAt + 2)
            {
                ...
                // CW_PM4_INDEX_ADDR_SWIZZLE=1 restores the old reading — the
                // same-binary control arm for the fix.
                static const bool oldReading = getenv("CW_PM4_INDEX_ADDR_SWIZZLE") != nullptr;
                ...
                // CW_PM4_DRAW_TRACE=1 — the raw DRAW_INDX body for the first few
            ...
            // CW_PM4_NO_TILE_OFFSET=1 is the control arm.
            {
                static const bool noTileOffset = getenv("CW_PM4_NO_TILE_OFFSET") != nullptr;
        ...
        case 0x3C: // WAIT_REG_MEM: wait_info, addr/reg(+endian), ref, mask, [interval]
        {
            ...
            if (!met)
            {
                ...
                // runs behind that). CW_PM4_NO_STOP_ON_WAIT=1 is the control arm.
                //
                ...
                // deliberately parked with CW_ISR_SINGLE_CPU=1. Both stall sites also
                   ...
                   // clock — VdSwap is kHighFrequency and appears in no kernel log.
        {
            ...
            if (bodyCount >= 4 && body(0) == 0x53574150 /* 'SWAP' */)
            {
                ...
                // A no-op unless CW_DETERMINISTIC_CLOCK is set.
                cw_timebase::AdvanceFrame();
            }
            break;
        }
        ...
        case 0x3F: // INDIRECT_BUFFER: physical address, size in dwords
            if (bodyCount >= 2 && depth < 8)
            {
                ...
                // there. The per-tile draw census (CW_VK_DRAW_CENSUS_EVERY) showed
                ...
                // with this, 15-35% without. CW_PM4_NO_REPLAY_RESTORE=1 is the control.
                static const bool noReplayRestore = getenv("CW_PM4_NO_REPLAY_RESTORE") != nullptr;
    ...
    while (pos < sizeDwords)
    {
        ...
        if (!consumed)
        {
            ...
            if (n < 16 || (n & 0xFFFu) == 0)
            {
                ...
                // belongs to. Use CW_PM4_DUMP_TRUNCATED for the whole thing.
                ...
                // CW_PM4_DUMP_TRUNCATED=<path>: the whole buffer, once. The trail above
                ...
                // find it.
                if (const char* path = getenv("CW_PM4_DUMP_TRUNCATED"))
                {
...
uint32_t Pm4_Execute(uint8_t* base, uint32_t writePtr)
{
    ...
    if (g_stallHit)
    {
        ...
        if (n <= 4 || (n & 0xFFFFF) == 0)
            fprintf(stderr, "[pm4] CW_PM4_STOP_ON_WAIT: ring held at an unsatisfied "
    ...
    // CW_PM4_RESYNC=1 enables a forward scan to the next plausible packet header for
    ...
    // a second" only because the pump ticked at the vblank cadence — and CW_PM4_TICK_MS
