...
// `cw_runtime --translate-shaders` against assets/shader_spv is the check (run it
...
// verification arms (CW_VK_VERIFY_CONST_GATHER and its poison) without re-running them.
#include "xenos_pch.h"
...
#include "cw_shader_common_embedded.h" // generated at configure time; see CMakeLists
...
static void LoadDxcOnce()
{
    ...
    if (const char* env = getenv("CW_DXC_LIB"); env && *env)
    ...
    g_dxcErr = "no dxcompiler library found (tried CW_DXC_LIB, <exe>/lib, <exe>, "
                         ...
                         std::vector<uint8_t>& spv, std::string& err)
{
    ...
    // CW_DXC_DEFINES passthrough, same contract as the shell pipeline: extra
    // whitespace-separated tokens, so an arm cache can be built from the same
    // translator into a second directory.
    if (const char* extra = getenv("CW_DXC_DEFINES"); extra && *extra)
    {
        std::string tok;
...
int TranslateDirectory(const char* ucodeDir, const char* outDir)
{
    ...
    const char* keepHlsl = getenv("CW_TRANSLATE_KEEP_HLSL");
