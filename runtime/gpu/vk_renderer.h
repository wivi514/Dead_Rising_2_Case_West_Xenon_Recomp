...
//
// OFF BY DEFAULT UNTIL IT IS GOOD ENOUGH TO BE ON: CW_VKDRAW=1 enables it. That is not
...
// false and stays false when CW_VKDRAW is unset, when there is no usable Vulkan
...
// Part 117: the same draw from the two-core pump's `cw-draw` thread, with the register
...
// The periodic counter block (CW_VK_STATS=1, and once at exit). Every "we could not
...
// `VkRenderer_DumpStats`, which `CW_VK_STATS` also calls every N frames: saving there
...
// `cw_runtime --diag` (part 105): every physical device, its driver, the requirements
...
// feeds can be live in a run: CW_VKDRAW=1 activates the PM4 feed and makes these
// no-ops; CW_D3D_DRAW=1 activates these and makes VkRenderer_Draw/OnSwap no-ops. The
