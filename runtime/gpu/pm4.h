...
// The per-register ALU constant write histogram (CW_PM4_ALU_WRITE_CENSUS=1, part 87)
...
// only under CW_PM4_IB_VERIFY.
uint64_t Pm4_IbTruncatedCount();
uint64_t Pm4_IbVerifyCleanCount();
uint64_t Pm4_IbVerifyDirtyCount();

// WAIT_REG_MEM health, and the numbers that decide whether CW_PM4_STOP_ON_WAIT is
...
// Only CW_PM4_FENCE_MONOTONIC (a phase C part 7 EXPERIMENT arm) acts on it; the count
