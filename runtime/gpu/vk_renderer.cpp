...
// instruments do, and all three are env-rooted: `CW_VK_PSBIND`, `CW_VK_DRAW_CENSUS`
// (armed by F9, but only ever when the variable is set) and `CW_VK_RESOLVE_TRACE`.
...
// --- CW_VK_PROFILE=N — where a FRAME's CPU time actually goes ------------------------
//
...
struct ProfilePhases
{
    ...
    // CW_VK_FRAME_STATS's OWN COST, and it exists because part 50 wrote the rule and
    ...
    // `CW_VK_PROFILE`, whose 2-4 ms bill was found by reading `CW_VK_FRAME_STATS`
...
// without `CW_VK_PROFILE`, so "removing" it would save a frame time nobody is paying.
...
uint32_t g_extraScopes = 0;   // CW_VK_PROFILE_EXTRA_SCOPES — the control; see DoDraw
...
// --- CW_VK_TEX_GUARD / CW_VK_TEX_REVALIDATE ------------------------------------------
//
...
//
// The mechanism the code already half-admits, in the CW_VK_TEX_REFRESH comment: the
...
//
// `CW_VK_TEX_REFRESH` was built for one instance of this (a font atlas the CPU keeps
...
// CW_VK_TEX_GUARD_EVERY_FETCH=1 — the same-binary CONTROL ARM for that policy, i.e. the
...
// CW_VK_DIM_CENSUS=1 — WHERE IS THE DIMENSION IN THE TEXTURE FETCH CONSTANT?
//
...
// CW_VK_DIM_DISAGREE=N — PRINT THE FIRST N SHADER-VERSUS-CONSTANT DISAGREEMENTS IN FULL.
//
...
// CW_VK_STREAM_CENSUS=1|2 — what the per-frame vertex/index stream cache actually does.
//
...
// CW_VK_STREAM_CENSUS_POISON=1 — THE CONTROL ARM FOR THE CONTENT CHECK, and it exists
...
// simply declines to check. `CW_VK_STREAM_CENSUS=2` measures what the sampling misses:
...
// asymmetry, guess high. `CW_VK_STREAM_GUARD_BYTES=N` overrides it without a rebuild so
// the bound can be tuned against the defect instead of against a model of it, and
// `CW_VK_STREAM_GUARD_EXACT=1` still means unlimited.
//
...
// CW_VK_GUARD_FOLD_SERIAL=1 restores the single-accumulator fold — the same-binary
...
inline bool GuardFoldSerial()
{
    ...
    static const bool serial = getenv("CW_VK_GUARD_FOLD_SERIAL") != nullptr;
    return serial;
}
...
// CW_VK_STREAM_GUARD_EXACT=1 — hash EVERY byte, whatever the stream's size.
//
...
// guilty", which `CW_VK_NO_PERSIST_STREAMS=1` cannot: that one disables the store
...
// "raise the bound" stayed unactionable for two parts in Case Zero.
...
// The bound the TEXTURE content guard folds at, in bytes. `CW_VK_TEX_GUARD_BYTES=N`.
//
...
// Which feed owns the renderer this run. False = the PM4 executor (CW_VKDRAW,
// phase 5); true = the D3D draw service (CW_D3D_DRAW, phase C). Set once at init and
...
// ===================================================================================
// INTERNAL RESOLUTION SCALE — CW_VK_RES / CW_VK_RES_SCALE
// ===================================================================================
//
...
// class (a stale mesh, a stale HUD), so it is not waved away: `CW_VK_VERIFY_PARALLEL_GUARD=1`
// measures how often it actually happens, and it is what must be read before this is
// called free. `CW_VK_NO_PARALLEL_GUARD=1` is the same-binary control arm.
//
...
// The census, printed with CW_VK_PROFILE. `served` over `requests` is the hit rate the
...
// CW_VK_VERIFY_PARALLEL_GUARD=1 — hash inline as well and compare, on every served
...
// CW_VK_VERIFY_PARALLEL_GUARD_POISON=1 — perturb every pre-hash so the check MUST fire.
...
unsigned GuardPoolWorkers()
{
    static const unsigned n = [] () -> unsigned {
        if (EnvOn("CW_VK_NO_PARALLEL_GUARD"))
            return 0;
        ...
        // usable, and `CW_VK_GUARD_WORKERS=4` is the arm that restores part 53's number.
        ...
        return ThreadBudget_Take("guard", 4, "CW_VK_GUARD_WORKERS");
    }();
    return n;
}
...
void GuardPoolDispatch()
{
    ...
    if (!g_gp)
    {
        g_gp = new GuardPool();
        g_gp->workers = n;
        g_gpVerify = EnvOn("CW_VK_VERIFY_PARALLEL_GUARD");
        g_gpVerifyPoison = EnvOn("CW_VK_VERIFY_PARALLEL_GUARD_POISON");
    }
    GuardPool& gp = *g_gp;
    if (gp.gather.empty())
        return;
...
extern "C" uint32_t CW_DebugWritePlayerObject(FILE* f, uint32_t bytes);
...
// CW_VK_FETCH_SLOT_INVERT=1 — read vertex fetch constants at `95 - slot`.
//
...
{
    static const bool invert = getenv("CW_VK_FETCH_SLOT_INVERT") != nullptr;
...
// places. `CW_VK_NO_FLAT_CACHE=1` keeps the `std::unordered_map` and is the same-binary
// control arm; `CW_VK_VERIFY_FLAT_CACHE=1` maintains BOTH structures and compares every
// lookup, and `CW_VK_VERIFY_FLAT_CACHE_POISON=1` perturbs the flat one so the check can
...
// THE SWAPCHAIN, when CW_VK_SWAPCHAIN=1 puts one on the window (part 54, plan §7).
//
...
struct SwapchainState
{
    ...
    // CW_VK_SWAPCHAIN_DUMP — the picture gate for this arm. See CreateSwapchain.
...
struct Renderer
{
    ...
    // Null unless CW_VK_VALIDATION=1 brought VK_EXT_debug_utils in with the layer. See
    ...
    struct PersistEntry
    {
        ...
        //
        // IMPORTED FROM CASE ZERO part 46, commit 82d181f (2026-08-15), on 2026-08-16.
        // See docs/imported-fixes.md — if Case Zero revises this, that file says exactly
        // what this port carries so the delta can be taken rather than re-derived.
        //
        // The guard is exact up to a byte bound and sampled above it, and the UI text
        // layer — one big vertex buffer the guest sub-allocates every run of glyphs out
        // of — sits above any affordable bound: Case Zero raising it to 256 KB left the
        // HUD still dropping out, while an unlimited arm fixed it, which places the
        // buffer above 256 KB. Buying exactness by SIZE therefore costs 121 MB/frame at
        // 256 KB and more above it, to protect a handful of buffers.
        //
        ...
        // the sampled guard. That hole is REAL and Case Zero's operator hit it on the
        // first session with this policy: "UI did break at the start of being in game but
        // then it seems to be good now" — the promotion is earned, so until a stream has
        // been caught changing once it is still sampled, and a miss in that window serves
        // the stale buffer exactly as before. It then self-heals, which is why the defect
    ...
    //
    // The budget is not a refinement, it is the whole thing working. Without it Case
    // Zero measured the probe at 838 streams and 66.8 MB/frame on its outdoor route — as
    // much as hashing everything exactly, which is the cost this policy exists to avoid —
    // because a streaming world meets new geometry continuously, so "new entries only" is
    // not a small population at all. With it the probe is a fixed toll paid to the
    ...
    //
    // NOTE FOR CASE WEST: those costs are CASE ZERO's, measured on ITS outdoor route.
    // This title has no outdoors and streams a facility interior, so the population that
    // made the unbounded version expensive may well be smaller here. The budget is kept
    // at Case Zero's value because it is a ceiling rather than a target — but the
    // `[vkprof] guard PROMOTED` line below prints this port's own numbers, and they have
    ...
    // look exactly like a defect in the thing being verified. So `CW_VK_NO_FLAT_CACHE=1`
    ...
    // ON by default, with `CW_VK_NO_PERSIST_STREAMS=1` as the control arm — the same
    // shape as CW_VK_NO_ARENA_GROWTH and CW_VK_NO_SUBMIT, so one binary is both arms of
    ...
    // one place a fresh command buffer starts. CW_VK_NO_STATE_CACHE=1 is the arm.
    struct BoundState
    {
    ...
    // without pricing it.
    //
    // The map is kept for `CW_VK_NO_FLAT_CACHE=1` and for the verify arm, exactly as the
    ...
    // containers is populated in a given run (`CW_VK_NO_FLAT_CACHE=1` chooses), because
    ...
    // CW_VK_EXPOSURE_TRACE — this frame's spread of the title's own exposure scalar,
    // `pc(14).w`. It exists because part 31 made that number load-bearing and there was
    // no way to read it for a NAMED frame: `CW_VK_PSBIND` prints it, but it dedupes on
    ...
    // and is used by `CW_VK_NO_FLAT_CACHE=1` (the control arm) and by
    // `CW_VK_VERIFY_FLAT_CACHE=1` (both structures maintained, every lookup compared).
...
VkMemoryPropertyFlags ReadbackMemoryProps()
{
    ...
    // CW_VK_READBACK_UNCACHED=1 — the pre-fix arm, i.e. the write-combined readback.
    // The same-binary control for the frame-rate claim this change makes.
    if (EnvOn("CW_VK_READBACK_UNCACHED"))
    {
...
//
// `CW_VK_NO_VRAM_STREAMS=1` is the same-binary control arm and puts them back in RAM.
...
// handle. A no-op without CW_VK_VALIDATION — the function pointer is null then, and the
...
VkDeviceSize ArenaAlloc(VkDeviceSize bytes, VkDeviceSize align = 256)
{
    ...
    if (at + bytes > R->arenaLimit)
    {
        ...
        static const bool noGrowth = EnvOn("CW_VK_NO_ARENA_GROWTH");
        if (!noGrowth)
...
bool CreateDevice()
{
    ...
    app.pApplicationName = "cw_runtime";
    app.apiVersion = VK_API_VERSION_1_3;

    VkInstanceCreateInfo ici{ VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO };
    ici.pApplicationInfo = &app;

    // CW_VK_VALIDATION=1 turns on the validation layer. Off by default because it is
    ...
    const bool wantValidation = EnvOn("CW_VK_VALIDATION");
    // The swapchain is the DEFAULT present path since part 54; `CW_VK_NO_SWAPCHAIN=1` is
    ...
    if (ir == VK_ERROR_EXTENSION_NOT_PRESENT && wantValidation)
    {
        ...
        // the surface extensions and leave CW_VK_SWAPCHAIN asking for a swapchain on an
    ...
    // LRU this still needs. CW_VK_MAX_TEXTURES overrides it — including DOWNWARD, which
    // is the same-binary arm that reproduces the exhaustion on demand.
    {
        ...
        if (const char* env = getenv("CW_VK_MAX_TEXTURES"))
    ...
    // baseline) therefore says NOTHING about per-stream overruns; CW_VK_RANGE_CENSUS is
    ...
    if (Env("CW_VK_ROBUST"))
    {
        ...
        if (have.features.robustBufferAccess)
        {
            ...
            fprintf(stderr, "[vk] CW_VK_ROBUST: robustBufferAccess ENABLED\n");
        }
        else
            fprintf(stderr, "[vk] CW_VK_ROBUST asked, but the device does not report "
    ...
    // That keeps the two arms one binary and keeps `CW_VK_FRAMES_IN_FLIGHT=1` byte-for-
...
bool LoadShaders()
{
    ...
    // than pick one, and let CW_SHADER_SPV override — a renderer that came up with
    ...
    if (const char* env = Env("CW_SHADER_SPV"))
        dir = env;
    else
    {
        ...
        if (dir.empty())
        {
            fprintf(stderr,
                    ...
                    "     (cd runtime/build && CW_SHADER_DUMP=/tmp/ucode ./cw_runtime)\n"
...
TextureFetch DecodeTextureFetch(const uint32_t* regs, uint32_t slot)
{
    ...
    // repeating for Case West because it costs one run. `CW_VK_DIM_CENSUS=1` partitions
...
// a picture. CW_VK_SNAPSHOT_MAX_AGE=N restores a bounded window (1 = the pre-part-15
// behaviour) as the same-binary control arm.
uint64_t SnapshotMaxAge()
{
    static const uint64_t age =
        Env("CW_VK_SNAPSHOT_MAX_AGE")
            ? strtoull(Env("CW_VK_SNAPSHOT_MAX_AGE"), nullptr, 10)
            : 0;   // 0 = no limit
    return age;
}
...
//
// CW_VK_NO_CUBE_SNAPSHOT=1 declines to the dummy, i.e. the pre-part-26 renderer, in the
...
{
    static const bool disabled = EnvOn("CW_VK_NO_CUBE_SNAPSHOT");
    if (disabled)
    {
        Count("texture: CUBE at a resolve destination declined (CW_VK_NO_CUBE_SNAPSHOT)");
        return 0;
    }
                       ...
                       uint32_t shaderDim)
{
    ...
    //
    // CW_VK_TEX_CACHE_FIRST=1 restores the old order — the same-binary control arm for
    ...
    //
    // NOT on the CW_VK_NO_CUBE arm, where the caller has deliberately lied about the
    ...
    static const bool noCubeArm = EnvOn("CW_VK_NO_CUBE");
    if (t.dimension != shaderDim && !noCubeArm)
    {
        ...
        //
        // CW_VK_NO_CUBE_REPLICATE=1 restores the decline — the same-binary control arm,
        ...
        if (cubeFetch)
        {
            static const bool noReplicate = EnvOn("CW_VK_NO_CUBE_REPLICATE");
            if (noReplicate)
            {
                COUNT("texture: CUBE fetch whose constant says otherwise — served the "
                      "dummy (CW_VK_NO_CUBE_REPLICATE)");
                return 0;
            }
    ...
    static const bool cacheFirst = EnvOn("CW_VK_TEX_CACHE_FIRST");
    if (cacheFirst)
    {
    ...
    // `CW_VK_NO_CUBE_SNAPSHOT=1` is the dummy, i.e. the pre-part-26 picture, and
    // `CW_VK_CUBE_FROM_GUEST=1` is the zeros in guest memory.
    //
    ...
    if (cubeFetch && R->snapshots.count(t.address & 0x1FFFFFFF))
    {
        ...
        static const bool cubeFromGuest = EnvOn("CW_VK_CUBE_FROM_GUEST");
        if (!cubeFromGuest)
            cubeAtResolveDest = true;
        else
            COUNT("texture: CUBE fetch at a resolve destination, uploaded from guest "
                  "memory anyway (CW_VK_CUBE_FROM_GUEST)");
    }
    if (!cubeFetch)
    {
        ...
        // CW_VK_NO_DEPTH_FETCH=1 — serve EVERY depth-format fetch the 1x1 white dummy.
        ...
        // with CW_VK_SKIP_TEX instead.
        static const bool noDepthFetch = EnvOn("CW_VK_NO_DEPTH_FETCH");
        if (noDepthFetch && wantsDepth)
        {
            COUNT("texture: depth fetch forced to the white dummy "
                  "(CW_VK_NO_DEPTH_FETCH)");
            return 0;
        }
        if (snap != R->snapshots.end() && SnapshotUsable(snap->second))
        {
            ...
            // the counter is what said which (gotcha 80). CW_VK_NO_SNAPSHOT_VIEWS=1 is
            // the same-binary control arm.
            static const bool noViews = EnvOn("CW_VK_NO_SNAPSHOT_VIEWS");
            if (t.width && t.height &&
            ...
            // two instruments — the `(snap)` suffix on `[psbind]`/`CW_VK_DRAW_CENSUS`
            // lines and the `sampled snapshots:` line of `CW_VK_RESOLVE_TRACE` — and it
    ...
    // CW_VK_TEX_REFRESH=<hex[,hex...]> — re-read these textures' pixels on every fetch,
    ...
    // CW_VK_TEX_REFRESH_ALL=1 — the same thing for EVERY texture, which is the arm the
    ...
    static const char* refreshEnv = Env("CW_VK_TEX_REFRESH");
    static const bool refreshAll = EnvOn("CW_VK_TEX_REFRESH_ALL");
    ...
    if (cached && !refresh)
    {
        ...
        // TEXTURE is. See the CW_VK_TEX_GUARD comment for why the two differ.
        //
        ...
        // as "already validated in frame 0". `CW_VK_TEX_GUARD_EVERY_FETCH=1` is the
    ...
    // CW_VK_NO_TEX_SWIZZLE=1 restores the identity mapping, so the change is
    ...
    static const bool noSwizzle = EnvOn("CW_VK_NO_TEX_SWIZZLE");
    ...
    // check on it is the census below plus `CW_VK_TEX_DUMP`, which writes each face out
    ...
    //
    // CW_VK_NO_MIPS=1 uploads level 0 alone — the pre-part-39 renderer, same binary.
    static const bool noMips = EnvOn("CW_VK_NO_MIPS");
    ...
    if (!noMips && layers == 1 && t.mipAddress && t.mipMax >= 1)
    {
        ...
        for (uint32_t level = 1; level <= t.mipMax && level < 16; level++)
        {
            ...
            // CW_VK_NO_MIP_TAIL=1 is the tail-only same-binary arm: isTail never
            ...
            // on the mostly-empty or divergence check below). CW_VK_NO_MIPS=1
            // remains the whole-feature arm.
            static const bool noTail = EnvOn("CW_VK_NO_MIP_TAIL");
    ...
    // CW_VK_MIP_TINT=1 — replace every uploaded chain level's blocks with a solid
    ...
    static const bool mipTint = EnvOn("CW_VK_MIP_TINT");
    if (mipTint && (t.format == xenos::kFmt_DXT1 || t.format == xenos::kFmt_DXT4_5))
    {
        ...
        Count("texture: mip levels TINTED (CW_VK_MIP_TINT)");
    }

    // CW_VK_TEX_DUMP=<dir> plus CW_VK_TEX_DUMP_ADDR=<hex[,hex]> — write the UNTILED
    ...
    static const char* texDumpDir = Env("CW_VK_TEX_DUMP");
    static const char* texDumpAddr = Env("CW_VK_TEX_DUMP_ADDR");
    ...
    // CW_VK_TEX_DUMP does not format a string per fetch.
    char dumpAddrHex[16] = {};
    ...
    if (texDumpDir && (!texDumpAddr || strstr(texDumpAddr, dumpAddrHex)))
    {
        ...
        Count("texture: dumped for CW_VK_TEX_DUMP");
    ...
    if (refresh && cached)
    {
        ...
        // CW_VK_TEX_REFRESH_ALL — which bypasses the guard block entirely — would leave
        ...
        if (pixels.size() <= R->staging.size)
        {
            ...
            Count("texture: refreshed in place (CW_VK_TEX_REFRESH)");
        }
        return cached->slot;
    }
...
// Pipelines
// ===================================================================================
// CW_VK_NO_PIPELINE_CACHE1=1 disables the one-entry front cache below — the same-binary
...
{
    static const bool off = getenv("CW_VK_NO_PIPELINE_CACHE1") != nullptr;
    return off;
}
...
VkPipeline GetPipeline(const PipelineKey& key, const ShaderMeta& vs, const ShaderMeta& ps)
{
    ...
    // CW_VK_NO_DEPTH_TEST=1 — an ARM, never a fix: draw everything regardless of
    ...
    static const bool noDepthTest = getenv("CW_VK_NO_DEPTH_TEST") != nullptr;
    ds.depthTestEnable =
    ...
    static const bool depthAlways = EnvOn("CW_VK_DEPTH_ALWAYS");
    if (depthAlways)
...
void BeginFrame()
{
    ...
    if (g_streamCensus)
    {
        ...
        // silently empty this census in the `CW_VK_NO_FLAT_CACHE` arm, and a census that
...
void SubmitFrame()
{
    ...
    // from it (`CW_VK_FRAME_STATS`'s coverage, luma and hashes) is meaningless. Read the
    // `msec` column and nothing else.
    static const bool noSubmit = EnvOn("CW_VK_NO_SUBMIT");
    if (noSubmit)
    {
        Count("submit: SKIPPED (CW_VK_NO_SUBMIT — ceiling measurement, picture invalid)");
        return;
    }
...
// ===================================================================================
// THE SWAPCHAIN (CW_VK_SWAPCHAIN=1) — plan §7, built in part 54
// ===================================================================================
...
// `CW_VK_SWAPCHAIN_FIFO=1` selects it deliberately, as the arm for exactly that question.
...
bool CreateSwapchain(uint32_t wantW, uint32_t wantH)
{
    ...
    // CW_VK_SWAPCHAIN_DUMP=<dir> — read the SWAPCHAIN IMAGE back and write it as a PPM.
    //
    ...
    if (Env("CW_VK_SWAPCHAIN_DUMP"))
    {
        if (caps.supportedUsageFlags & VK_IMAGE_USAGE_TRANSFER_SRC_BIT)
            sci.imageUsage |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        else
            fprintf(stderr, "[vk] CW_VK_SWAPCHAIN_DUMP: this surface does not allow "
    ...
    R->swap.rebuilds++;
    if (!R->swap.dumpDir)
        if (const char* d = Env("CW_VK_SWAPCHAIN_DUMP"))
        {
            std::error_code ec;
            std::filesystem::create_directories(d, ec);
            R->swap.dumpDir = d;
            if (const char* e = Env("CW_VK_SWAPCHAIN_DUMP_EVERY"))
                R->swap.dumpEvery = std::max(1ull, strtoull(e, nullptr, 10));
            fprintf(stderr, "[vk] CW_VK_SWAPCHAIN_DUMP: every %llu-th SWAPCHAIN IMAGE "
    ...
    fprintf(stderr,
            ...
            EnvOn("CW_VK_SWAPCHAIN_FIFO") ? " (CW_VK_SWAPCHAIN_FIFO=1)" : "");
    return true;
}
...
void RecordSwapchainBlit(Image& source, uint32_t width, uint32_t height)
{
    ...
    // resolution, and those stopped being the same number the moment CW_VK_RES existed.
...
void PresentSwapchain()
{
    ...
    if (R->swap.dumpPending)
    {
        ...
        if (FILE* f = fopen(path, "wb"))
        {
            ...
            fprintf(stderr, "[vk] CW_VK_SWAPCHAIN_DUMP: cannot write %s\n", path);
    }
}
...
int RetireOldestFrame()
{
    ...
    // CW_VK_NO_SUBMIT recorded a frame and executed none of it, so no fence will ever
                       ...
                       int kind)
{
    ...
    if (R->persistOn)
    {
        ...
        // CW_VK_NO_DYNAMIC_GUARD=1 is the same-binary control arm: the pre-import policy,
        // where exactness is bought by SIZE alone and a big dynamic buffer is sampled
        // forever. Without an off switch this change could never be shown to be the thing
        // that fixed (or did not fix) the HUD — and on this port it has NOT yet been shown
        // to fix anything, because the defect was reported here but the A/B was run in
        // Case Zero.
        static const bool noDynamicGuard = EnvOn("CW_VK_NO_DYNAMIC_GUARD");
        ...
        if (!noDynamicGuard && pit)
        {
            ...
            //
            // CW_VK_GUARD_BUDGET=1 selects the cheaper policy: proven need
            ...
            // took the promotion from 35-48 MB/frame to 30.5. CW_VK_NO_GUARD_BUDGET=1
            // is the same-binary control arm (the session-3 policy: every stream ever
            // caught changing stays exact forever).
            static const bool budgetAll = !EnvOn("CW_VK_NO_GUARD_BUDGET");
            if (pit->dynamic &&
        ...
        if (g_streamCensus >= 2)
        {
            ...
            if (pit != g_prevStreamKeys.end())
            {
                ...
                else
                {
                    ...
                    // capable of reading otherwise: `CW_VK_STREAM_CENSUS_POISON=1` makes
                           ...
                           uint32_t& outCount)
{
    ...
    for (uint32_t p = 0; p < prims; p++)
    {
        ...
        else
        {
            ...
            // CW_VK_RECT_HALF=1 restores the old same-triangle-twice expansion — the
            ...
            static const bool half = EnvOn("CW_VK_RECT_HALF");
            if (half || prims != 1)
            {
                if (prims != 1)
...
// different bind from offset 0 of the other. CW_VK_NO_STATE_CACHE=1 disables this with
// the other five, so one arm remains the whole pre-cache renderer.
// CW_VK_NO_BUFFER_BIND_CACHE=1 is the ISOLATED control arm for this change, and it has
// to exist separately from CW_VK_NO_STATE_CACHE: that one also undoes part 18's five
...
bool NoBufferBindCache()
{
    static const bool off =
        Env("CW_VK_NO_BUFFER_BIND_CACHE") || Env("CW_VK_NO_STATE_CACHE");
    return off;
}
...
{
    static const bool off = EnvOn("CW_VK_NO_FETCH_SAMPLERS");
    if (off)
        return 0;
            ...
            const Pm4ShaderBinding& vsBind, const Pm4ShaderBinding& psBind)
{
    ...
    if (key.drawIdPass)
        Count("draw: painted its INDEX (CW_VK_DRAW_ID)");
    // CW_VK_FORCE_COLORMASK=1 — treat every draw as writing all four channels.
    //
    ...
    {
        static const bool noAlphaTest = EnvOn("CW_VK_NO_ALPHA_TEST");
    ...
    // CW_VK_PRIM_RESTART=1 is the arm. Deciding this needs a frame-aligned comparison
    ...
    static const bool wantRestart = EnvOn("CW_VK_PRIM_RESTART");
    key.primRestart = (wantRestart && restartable && draw.indexed) ? 1 : 0;

    // CW_VK_ONLY_VS=<hex[,hex...]> / CW_VK_SKIP_VS=<hex[,hex...]> — render only, or all
    ...
    {
        static const char* only = Env("CW_VK_ONLY_VS");
        static const char* skip = Env("CW_VK_SKIP_VS");
        if (only || skip)
        {
            char hex[24];
            ...
            {
                Count("draw: filtered out (CW_VK_ONLY_VS)");
                return;
            }
    ...
    // CW_VK_SHADER_CENSUS=1 — draws per (vs, ps) pair, in the stats block. Which
    ...
    static const bool shaderCensus = EnvOn("CW_VK_SHADER_CENSUS");
    if (shaderCensus)
    {
        char name[64];
    ...
    // on the prediction. `CW_VK_NO_CONST_MEMO=1` is the same-binary control arm.
    //
    ...
    // `CW_VK_VERIFY_CONST_MEMO=1` does the copy anyway into a scratch buffer and compares
    ...
    _pBegin.Close();
    {
        ...
        // CW_VK_PS_CONST_SCALE="14.w=4,18.y=0.5" — multiply chosen PIXEL constant
        ...
        static const std::vector<PsConstScale> psConstScale = []
        {
            ...
            const char* spec = Env("CW_VK_PS_CONST_SCALE");
            if (!spec)
                return out;
            for (const char* p = spec; *p;)
            {
                ...
                if (index < 256 && comp < 4 && eq)
                {
                    ...
                    fprintf(stderr, "[vk] CW_VK_PS_CONST_SCALE: pc(%u).%c *= %g\n", index,
                            "xyzw"[comp], double(out.back().factor));
                }
                else
                {
                    fprintf(stderr, "[vk] CW_VK_PS_CONST_SCALE: cannot parse \"%s\" — "
        ...
        if (!psConstScale.empty())
        {
            ...
            Count("draw: a PIXEL constant was scaled by CW_VK_PS_CONST_SCALE");
        }
    ...
    // discarded.
    // CW_VK_PSBIND=<pshash> — what each of this pixel shader's samplers is actually
    ...
    static const char* psbindEnv = Env("CW_VK_PSBIND");
    bool psbind = false;
    if (psbindEnv)
    {
        char psbindWant[24];
    ...
    // CW_VK_DRAW_CENSUS=<file> plus F9 — EVERY draw of ONE frame, with what each one
    ...
    if (drawCensus && !R->drawCensusFile)
    {
        ...
        // a capture armed by CW_CAPTURE_KEY announced a census, ran the frame, and wrote
        // no file. Two places deciding where output goes is one place too many.
        static std::string capturePath;
        if (capturePath.empty())
            if (const char* d = Env("CW_CAPTURE_KEY"))
                capturePath = std::string(d) + "/capture.census";
        const char* censusEnv =
            capturePath.empty() ? Env("CW_VK_DRAW_CENSUS") : capturePath.c_str();
        if (const char* path = censusEnv)
        {
            ...
            if (R->drawCensusFile)
                ...
                fprintf(stderr, "[vk] cannot write CW_VK_DRAW_CENSUS -> %s\n", named);
        }
    }
    ...
    auto bindTextures = [&](const std::vector<uint32_t>& consts,
        ...
        for (size_t i = 0; i < consts.size(); i++)
        {
            ...
            // counters below sat after the CW_VK_NO_CUBE forcing, so on the very arm the
            ...
            // CW_VK_NO_CUBE=1 — bind every cube fetch the way the renderer did before
            ...
            static const bool noCube = EnvOn("CW_VK_NO_CUBE");
            if (noCube && dim == 3)
            {
                dim = 1;
                Count("texture: cube fetch forced back to the 2D array (CW_VK_NO_CUBE)");
            }
    ...
    // CW_VK_TEX_DUMP=<dir> plus CW_VK_TEX_DUMP_PS=<pixel shader hash> — write out the raw
    ...
    // route headlessly with CW_VK_TEX_DUMP_ADDR pointed at them — and got back a picture
    ...
    static const char* texDumpDirPs = Env("CW_VK_TEX_DUMP");
    static const char* texDumpPs = Env("CW_VK_TEX_DUMP_PS");
        ...
        if (strstr(texDumpPs, psHex))
        {
            for (uint32_t constIdx : ps.tfetchConsts)
            {
                ...
                if (FILE* f = fopen(path, "wb"))
                {
                    ...
                    fclose(f);
                    Count("texture: dumped for CW_VK_TEX_DUMP_PS");
                }
            }
        }
    }

    // CW_VK_ONLY_TEX / CW_VK_SKIP_TEX=<hex[,hex...]> — render only, or all but, the
    // draws whose first bound texture is at that guest address.
    //
    // The bisection arms one level down from CW_VK_ONLY_VS. A UI compose is a hundred
    ...
    //
    // CW_VK_TEX_FILTER_FILE=<path> is the same two arms, RE-READ WHILE THE GAME RUNS.
    ...
    {
        static const char* onlyTex = Env("CW_VK_ONLY_TEX");
        static const char* skipTex = Env("CW_VK_SKIP_TEX");
        static const char* filterFile = Env("CW_VK_TEX_FILTER_FILE");
        ...
        if (onlyEff || skipEff)
        {
            ...
            {
                Count("draw: filtered out (CW_VK_ONLY_TEX)");
                return;
            }
            if (skipEff && strstr(skipEff, hex))
            {
                Count("draw: filtered out (CW_VK_SKIP_TEX)");
                return;
            }
        }
    }
    if (psbind)
    {
        ...
        {
            const char* list = Env("CW_VK_PSBIND_PC");
    ...
    // statistic can see.
    //
    // CW_VK_TEXCOORD_SWAP=1 republishes the old mask — the same-binary control arm
    // that repaints the blotches. (CW_VK_NO_TEXCOORD_SWAP is accepted and now a no-op,
    ...
    {
        static const bool oldMask = EnvOn("CW_VK_TEXCOORD_SWAP");
        uint32_t swapped = 0;
        if (oldMask)
        {
            for (const VertexAttribute& a : vs.attributes)
    ...
    // without touching the shared XenosRecomp emitter. GEQUAL keeps the exact ref.
    {
        ...
        //
        // CW_VK_A2M_ANY_SURFACE=1 drops the 4x gate. It is a DIAGNOSTIC, not a fix:
        ...
        static const bool a2mAnySurface = EnvOn("CW_VK_A2M_ANY_SURFACE");
        const bool a2mSurface = a2mAnySurface ||
                                ((regs[xenos::kRbSurfaceInfo] >> 16) & 3) == 2;
        // CW_VK_A2M_MODE — 1 = FLAT threshold at 0.5, 2 = per-sample 2x2 dither.
        //
        ...
        static const uint32_t a2mMode = []() -> uint32_t {
            const char* m = Env("CW_VK_A2M_MODE");
    ...
    if (!(vte & 0x1))
    {
        ...
        //
        // CW_VK_WINDOW_COORDS_FRONT_BUFFER=1 restores the front buffer's extent — the
        // pre-part-15 behaviour, and the same-binary control arm for every claim about
        // the shadow cascade's bottom rows.
        static const bool windowCoordsFront = EnvOn("CW_VK_WINDOW_COORDS_FRONT_BUFFER");
        ...
        // CW_VK_NO_DEPTH_TEST=1, which fills every one of them.
        //
        // CW_VK_NO_MSAA_WINDOW_SCALE=1 is the same-binary control arm.
        static const bool noMsaaScale = EnvOn("CW_VK_NO_MSAA_WINDOW_SCALE");
        ...
        //
        // CW_VK_NO_MSAA_WINDOW_SCALE_Y=1 is the same-binary control arm (the part-33
        // renderer). The part-32 arm variable CW_VK_MSAA_WINDOW_SCALE_Y is RETIRED and
        ...
        static const bool noMsaaScaleY = EnvOn("CW_VK_NO_MSAA_WINDOW_SCALE_Y");
    ...
    //
    // CW_VK_HALF_PIXEL=1 restores the old value as the same-binary control arm.
    static const bool wantHalfPixel = EnvOn("CW_VK_HALF_PIXEL");
    ...
    static const bool noFlipY = EnvOn("CW_VK_NO_FLIP_Y");
    VkViewport viewport{};
    if (vte & 0x1)
    {
    ...
    static const bool viewportTrace = EnvOn("CW_VK_VIEWPORT_TRACE");
    if (viewportTrace)
    {
    ...
    static const bool stateProbe = EnvOn("CW_VK_STATE_PROBE");
    if (stateProbe)
    {
    ...
    // CW_VK_FETCH_PROBE=1 — which vertex fetch slots does the guest actually populate?
    //
    ...
    static const bool fetchProbe = EnvOn("CW_VK_FETCH_PROBE");
    ...
    //
    // CW_VK_NO_INDX_OFFSET=1 is the same-binary control arm: the pre-fix renderer.
    static const bool noIndxOffset = EnvOn("CW_VK_NO_INDX_OFFSET");
    const int32_t indxOffset =
        noIndxOffset ? 0 : int32_t(regs[xenos::kVgtIndxOffset] & 0xFFFFFF);

    static const bool rectHalf = EnvOn("CW_VK_RECT_HALF");
    ...
    static const char* const drawProbeEnv = Env("CW_VK_DRAW_PROBE");
    if (const char* probe = drawProbeEnv)
    {
        // CW_VK_DRAW_PROBE_MINVERTS bounds the probe to the big meshes. The first three
        ...
        static const uint32_t minVerts =
            Env("CW_VK_DRAW_PROBE_MINVERTS")
                ? uint32_t(atoi(Env("CW_VK_DRAW_PROBE_MINVERTS")))
                : 0;
        // CW_VK_DRAW_PROBE_MINFRAME bounds the probe to a steady-state frame, and it is
        ...
        static const uint64_t minFrame =
            Env("CW_VK_DRAW_PROBE_MINFRAME")
                ? uint64_t(atoll(Env("CW_VK_DRAW_PROBE_MINFRAME")))
                : 0;
        // CW_VK_DRAW_PROBE_COUNT=N — how many draws to print (default 3). Three is
        ...
        static int left = Env("CW_VK_DRAW_PROBE_COUNT")
            ...
            R->frame >= minFrame && left-- > 0)
        {
            ...
            // CW_VK_ONLY_TEX exists for — and a probe that does not name it produces two
            ...
            // CW_VK_DRAW_PROBE_PC=a,b,c — which pixel-shader constants to print.
            ...
            {
                const char* list = Env("CW_VK_DRAW_PROBE_PC");
            ...
            for (const VertexAttribute& a : vs.attributes)
            {
                ...
                if (a.location != 0 && !a.indirect)
                {
                    ...
                    // CW_VK_DRAW_PROBE_VERTS=N — how many vertices to print (default 4).
                    ...
                    static const uint32_t probeVerts =
                        Env("CW_VK_DRAW_PROBE_VERTS")
                            ? uint32_t(atoi(Env("CW_VK_DRAW_PROBE_VERTS")))
                            : 4;
    ...
    // CW_VK_PASS_DRAWS=N — how many of a pass's draws the resolve trace lists. Four
    ...
    static const size_t passDraws =
        Env("CW_VK_PASS_DRAWS") ? strtoul(Env("CW_VK_PASS_DRAWS"), nullptr, 10) : 4;
...
void DoResolve(uint8_t* base, const uint32_t* regs)
{
    ...
    // fifth of them. `CW_VK_NO_DEPTH_RESOLVE=1` is the same-binary control arm for
    ...
    // CW_VK_RESOLVE_TRACE=1 — one line per resolve for a few frames, with the surface
    ...
    // CW_VK_RESOLVE_TRACE=N starts at frame N (1 = from the beginning). The boot's
    // first frames are not the frame anyone is investigating, and 60 lines of them is
    // all a from-the-start trace ever shows.
    //
    // CW_VK_RESOLVE_TRACE_PASSES=N is the budget, in PASSES. It has been in CLAUDE.md
    ...
    static const int tracePasses =
        Env("CW_VK_RESOLVE_TRACE_PASSES")
            ? int(strtol(Env("CW_VK_RESOLVE_TRACE_PASSES"), nullptr, 10))
            : 20;
    ...
    static const bool smallEdram = EnvOn("CW_VK_SMALL_EDRAM");
    ...
    uint32_t dstX = copyX;
    uint32_t dstY = copyY;
    static const bool noAddrFold = EnvOn("CW_VK_NO_ADDR_TILE_FOLD");
    ...
    if (clearColor)
    {
        ...
        // CW_VK_CLEAR_POISON=1 — clear the colour target to MAGENTA instead of the guest's
        ...
        static const bool clearPoison = EnvOn("CW_VK_CLEAR_POISON");
        {
    ...
    if (clearDepth)
    {
        ...
        // CW_VK_DEPTH_CLEAR_FAR=1 — clear depth to 1.0 whatever RB_DEPTH_CLEAR says.
        ...
        static const bool clearFar = EnvOn("CW_VK_DEPTH_CLEAR_FAR");
        if (clearFar)
            value.depth = 1.0f;
        ...
        // CW_VK_SCOPED_CLEAR=1 — clear only the region THIS pass rendered, not the whole
        // EDRAM stand-in.
        //
        ...
        // ALWAYS (CW_VK_DEPTH_ALWAYS) fills it, so the geometry was always being
        ...
        static const bool scopedClear = EnvOn("CW_VK_SCOPED_CLEAR");
...
bool InitCommon()
{
    ...
    // before anything could sample them. `CW_VK_SMALL_EDRAM=1` restores the old size and
    ...
    // port up to part 18 was taken at, so `CW_VK_NO_ARENA_GROWTH=1` reproduces the old
    // renderer exactly and remains a usable control arm. CW_VK_ARENA_MB=N sets the
    // start, which is how the 128-vs-512 A/B that identified the black frames was run.
    static const uint64_t arenaMb =
        Env("CW_VK_ARENA_MB") ? strtoull(Env("CW_VK_ARENA_MB"), nullptr, 10) : 128;
    ...
    // CW_VK_PERSIST_MB=N sets the start.
    static const uint64_t persistMb =
        Env("CW_VK_PERSIST_MB") ? strtoull(Env("CW_VK_PERSIST_MB"), nullptr, 10) : 128;
    R->persistOn = !EnvOn("CW_VK_NO_PERSIST_STREAMS");
    ...
    if (const char* gb = Env("CW_VK_STREAM_GUARD_BYTES"))
    {
        ...
        if (v >= kGuardBlocks)
        {
            ...
            fprintf(stderr, "[vk] CW_VK_STREAM_GUARD_BYTES=%llu — the store's guard is "
                            "EXACT up to %llu bytes and samples above it (default %zu)\n",
                    v, v, kGuardBytesDefault);
        }
        else
        {
            fprintf(stderr, "[vk] CW_VK_STREAM_GUARD_BYTES=%llu REFUSED — must be >= %zu; "
    ...
    else
    {
        fprintf(stderr, "[vk] stream guard exact to %zu bytes, sampled above "
                        "(CW_VK_STREAM_GUARD_BYTES=N to change, item 00c)\n",
                g_guardBytes);
    }

    g_guardExact = EnvOn("CW_VK_STREAM_GUARD_EXACT");
    if (g_guardExact)
        fprintf(stderr, "[vk] CW_VK_STREAM_GUARD_EXACT=1 — the cross-frame store's guard "
                        "hashes EVERY byte, so it cannot miss a small edit in a large "
                        "stream. Diagnostic for open item 00c.\n");

    // --- CW_VK_FRAMES_IN_FLIGHT ---------------------------------------------------------
    //
    ...
    // (gotcha 231, §6ar). `CW_VK_NO_SUBMIT=1` measured the ceiling on removing that
    ...
    static const char* fifEnv = Env("CW_VK_FRAMES_IN_FLIGHT");
    ...
    // CW_VK_SNAP_ON_BLACK and CW_VK_SNAP_ON_DARK trigger a dump of the current
    // snapshots from a pixel test on the presented frame, and CW_VK_FRAME_STATS_SURFACE
    ...
    if (Env("CW_VK_SNAP_ON_BLACK") || Env("CW_VK_SNAP_ON_DARK") ||
        Env("CW_VK_FRAME_STATS_SURFACE") || Env("CW_VK_SNAP_DUMP") ||
        EnvOn("CW_VK_NO_SUBMIT"))
    {
        if (R->framesInFlight != 1)
    ...
    if (!CreateBuffer(R->arena, (arenaMb << 20) * R->framesInFlight,
        ...
        !CreateBuffer(R->readback,
                      ...
                      // CW_VK_SNAP_DUMP might read back — this title's shadow cascade
    ...
    // honoured; this sampler remains the fallback and the CW_VK_NO_FETCH_SAMPLERS
    // arm's whole world.
    fprintf(stderr, "[vk] per-fetch samplers %s (aniso device limit %.0fx)\n",
            Env("CW_VK_NO_FETCH_SAMPLERS") ? "OFF (CW_VK_NO_FETCH_SAMPLERS)" : "ON",
            R->anisoLimit);
    ...
    auto makeDummy = [&](Image& img, VkImageViewType type, uint32_t layers,
        ...
        // CW_VK_CUBE_POISON=1 — make the CUBE dummy opaque MAGENTA instead of white.
        //
        ...
        // CW_VK_NO_CUBE — was PIXEL-IDENTICAL on all 44 frames where both arms had the
        ...
        // the CONTENT of those cube maps. Same shape as CW_VK_TEX_GUARD_POISON
        // (gotcha 30) — a comparison that has never reported a positive proves nothing
        // by reporting a negative.
        // CW_VK_DUMMY_POISON=1 poisons ALL FOUR dummies, not just the cube one.
        //
        // CW_VK_CUBE_POISON answers "does a cube fetch reach the picture", and part 26
        ...
        static const bool cubePoison = EnvOn("CW_VK_CUBE_POISON");
        static const bool allPoison = EnvOn("CW_VK_DUMMY_POISON");
        const bool poisoned =
        ...
        if (poisoned)
            fprintf(stderr,
                    ...
                    allPoison ? "CW_VK_DUMMY_POISON" : "CW_VK_CUBE_POISON", setIndex,
                    layers);
    ...
    // CW_CAPTURE_KEY is in the list because it arms the draw census by another door —
    // an F9 press under it writes `capture.census` without CW_VK_DRAW_CENSUS being set,
    ...
    // correct, no reported slowdown. CW_VK_NO_TEX_REVALIDATE=1 is the same-binary
    // control arm that brings the random-texture defect back.
    {
        static const bool noRevalidate = EnvOn("CW_VK_NO_TEX_REVALIDATE");
        g_texGuard = !noRevalidate || EnvOn("CW_VK_TEX_GUARD");
    ...
    g_texGuardEveryFetch = EnvOn("CW_VK_TEX_GUARD_EVERY_FETCH");
    ...
    g_flatCacheOff = EnvOn("CW_VK_NO_FLAT_CACHE");
    g_flatCacheVerify = EnvOn("CW_VK_VERIFY_FLAT_CACHE");
    g_flatCacheVerifyPoison = EnvOn("CW_VK_VERIFY_FLAT_CACHE_POISON");
    ...
    if (const char* n = Env("CW_VK_TEX_GUARD_BYTES"))
    {
        ...
        fprintf(stderr,
                "[vk] CW_VK_TEX_GUARD_BYTES=%zu — the texture content guard is exact to "
                "that many bytes and samples 8 spread blocks above it (default 16384)\n",
                g_texGuardBytes);
    }
    if (g_texGuardEveryFetch)
        fprintf(stderr,
                "[vk] CW_VK_TEX_GUARD_EVERY_FETCH — the texture content guard runs on "
                "EVERY fetch, not once a frame per entry (the pre-part-47 cadence)\n");
    g_texGuardPoison = EnvOn("CW_VK_TEX_GUARD_POISON");
    if (g_texGuardPoison)
    ...
    if (EnvOn("CW_VK_TEX_REFRESH_ALL"))
        fprintf(stderr, "[vk] EVERY texture is re-read on EVERY fetch "
                        "(CW_VK_TEX_REFRESH_ALL) — a picture arm, ruinously slow, and "
                        "the cache cannot serve a stale image under it\n");
    g_profileOn = EnvOn("CW_VK_PROFILE");
    if (g_profileOn)
    {
        CalibrateProfNow();
        g_extraScopes = Env("CW_VK_PROFILE_EXTRA_SCOPES")
                            ? uint32_t(atoi(Env("CW_VK_PROFILE_EXTRA_SCOPES"))) : 0;
        if (g_extraScopes)
            fprintf(stderr,
                    "[vkprof] CW_VK_PROFILE_EXTRA_SCOPES=%u — the positive control for "
    ...
    g_streamCensus =
        Env("CW_VK_STREAM_CENSUS") ? atoi(Env("CW_VK_STREAM_CENSUS")) : 0;
    if (g_streamCensus && !g_profileOn)
    {
        fprintf(stderr, "[vk] CW_VK_STREAM_CENSUS needs CW_VK_PROFILE — it reports "
                        "through that window; census OFF\n");
        g_streamCensus = 0;
    }
    g_streamPoison = EnvOn("CW_VK_STREAM_CENSUS_POISON");
    if (g_streamPoison)
...
bool VkRenderer_Init()
{
    ...
    if (!EnvOn("CW_VKDRAW"))
    {
        fprintf(stderr, "[vk] renderer OFF (set CW_VKDRAW=1 to enable it)\n");
        return false;
    }
...
void DoSwapImpl(uint8_t* base, uint32_t frontBuffer, uint32_t width, uint32_t height)
{
    ...
    // CW_FPS_LOG=N — the frame rate, every N seconds, and NOTHING ELSE.
    //
    // It exists because every instrument this project owns that reports a frame rate has
    // a bill big enough to change the answer: `CW_VK_PROFILE` costs 2-4 ms a frame and
    // `CW_VK_FRAME_STATS` walks all 921,600 pixels for another 1.9-3.3 (gotcha 337). So
    ...
    // abandoning a frame here are knowable BEFORE the acquire — `CW_VK_NO_SUBMIT` records
    // a frame and executes none of it (the ceiling arm), and a command buffer that is not
    // recording cannot carry a blit — so the acquire is simply not made. `CW_VK_NO_SUBMIT`
    ...
    static const bool noSubmitArm = EnvOn("CW_VK_NO_SUBMIT");
    ...
    // deliberately, and CW_VK_READBACK_UNCACHED=1 is still the arm for that), so reading
    ...
    // (the frame dump, the E3 correlation, CW_VK_SNAP_*) sets one of them. Gating on
    ...
    // write-combined read. `CW_VK_PRESENT_STAGING=1` is the same-binary control arm.
    static const bool stagingCopy = !g_readbackCached || EnvOn("CW_VK_PRESENT_STAGING");
    ...
    // CW_VK_SNAP_ON_BLACK[=pct] — dump the whole resolve chain of the frame the picture
    // DIED on, triggered by the picture dying.
    //
    // The view-dependent whole-frame black is the port's top rendering defect and it has
    // never been captured, because CW_VK_SNAP_DUMP fires on a frame NUMBER and this
    ...
    // exactly one being wrong (the reason CW_VK_SNAP_DUMP exists at all).
    //
    ...
    // (gotcha 30). With `CW_VK_SNAP_ON_BLACK=99 CW_VK_SNAP_ON_BLACK_LIT=20` the second
    ...
    // few wasted files. CW_VK_SNAP_ON_BLACK_MAX raises it.
    static const char* onBlackEnv = Env("CW_VK_SNAP_ON_BLACK");
    static const double litPct =
        Env("CW_VK_SNAP_ON_BLACK_LIT") ? atof(Env("CW_VK_SNAP_ON_BLACK_LIT")) : 20.0;
    static int episodesLeft =
        Env("CW_VK_SNAP_ON_BLACK_MAX") ? atoi(Env("CW_VK_SNAP_ON_BLACK_MAX")) : 4;
    static bool sawLitFrame = false;
    static int onBlackBudget = 2;   // the transition frame, and the next one
    // CW_VK_SNAP_ON_DARK=<meanLuma> — the same trigger on the metric the defect ACTUALLY
    ...
    static const char* onDarkEnv = Env("CW_VK_SNAP_ON_DARK");
    static const double darkLitLuma =
        Env("CW_VK_SNAP_ON_DARK_LIT") ? atof(Env("CW_VK_SNAP_ON_DARK_LIT")) : 20.0;
    static int darkEpisodesLeft =
        Env("CW_VK_SNAP_ON_DARK_MAX") ? atoi(Env("CW_VK_SNAP_ON_DARK_MAX")) : 3;
    ...
    if (px && (onBlackEnv || onDarkEnv))
    {
        ...
        if (onDarkEnv)
        {
            ...
            // CW_VK_SNAP_ON_DARK_AFTER_MS=N — ignore everything before N ms of wall
            ...
            // that reach gameplay are written in seconds (CW_FAKE_START_MS intervals)
            ...
            static const long long afterMs =
                Env("CW_VK_SNAP_ON_DARK_AFTER_MS")
                    ? atoll(Env("CW_VK_SNAP_ON_DARK_AFTER_MS")) : 0;
    ...
    // CW_VK_FRAME_DUMP=<dir> writes every 64th frame as a PPM. This is the instrument
    ...
    // CW_VK_FRAME_DUMP_EVERY=N overrides the 64. A screen the synthetic-input arm walks
    ...
    if (px && R->capturePictureFrame && pres.frame == R->capturePictureFrame)
    {
        ...
        char path[512];
        snprintf(path, sizeof path, "%s/capture_%06llu.ppm", Env("CW_CAPTURE_KEY"),
        ...
        if (FILE* f = fopen(path, "wb"))
        {
            ...
            fprintf(stderr, "[vk] capture: wrote %s (%ux%u)%s\n", path, width1, height1,
                    R->drawIdRanOnFrame == pres.frame
                        ? " — NOT A PICTURE: CW_VK_DRAW_ID painted this frame's draw "
        ...
        snprintf(path, sizeof path, "%s/capture_%06llu.pose", Env("CW_CAPTURE_KEY"),
                 (unsigned long long)pres.frame);
        if (FILE* f = fopen(path, "w"))
        {
            ...
            float pos[3];
            long long ageMs = -1;
            if (CW_DebugPlayerPos(pos, &ageMs))
                ...
                fprintf(f, "# player_pos UNAVAILABLE — no level running, or the "
                           "lookup failed\n");
            const uint32_t obj = CW_DebugWritePlayerObject(f, 2048);
            fclose(f);
    ...
    static const char* dumpDir = Env("CW_VK_FRAME_DUMP");
    static const uint64_t dumpEvery =
        Env("CW_VK_FRAME_DUMP_EVERY")
            ? std::max<uint64_t>(1, strtoull(Env("CW_VK_FRAME_DUMP_EVERY"), nullptr, 10))
            : 64;
    if (px && dumpDir && (pres.frame % dumpEvery) == 0)
    {
        ...
        {
            complained = true;
            fprintf(stderr, "[vk] CW_VK_FRAME_DUMP cannot write %s — no frames will be "
                            "dumped this run\n",
                    path);
        }
    }

    // CW_VK_FRAME_STATS=<file> — one line per frame: what the guest asked for, and what
    ...
    // CW_VK_EXPOSURE_TRACE=<file> — one line per frame: how many draws set an exposure
    // and the range of values they set. Written here, at the present, so its frame
    // numbers are the same ones CW_VK_SNAP_FRAME and CW_VK_FRAME_STATS use.
    static FILE* expFile = nullptr;
    static bool expTried = false;
    if (!expTried)
    {
        expTried = true;
        if (const char* path = Env("CW_VK_EXPOSURE_TRACE"))
        {
            expFile = fopen(path, "w");
            if (expFile)
                fprintf(expFile, "# frame draws expMin expMax\n");
            else
                fprintf(stderr, "[vk] cannot write CW_VK_EXPOSURE_TRACE=%s\n", path);
        }
    }
    if (expFile)
    {
    ...
    {
        statsTried = true;
        if (const char* path = Env("CW_VK_FRAME_STATS"))
        {
            ...
            if (statsFile)
                ...
                fprintf(stderr, "[vk] cannot write CW_VK_FRAME_STATS=%s\n", path);
        }
    }
    // CW_VK_FRAME_STATS_SURFACE=<hex> — measure THAT resolve surface as well as the
    // presented frame.
    //
    ...
    // CW_VK_RESOLVE_TRACE names the right address for any era.
    static const char* surfaceEnv = Env("CW_VK_FRAME_STATS_SURFACE");
    ...
    // CW_VK_SNAP_DUMP=<dir> — write EVERY resolve snapshot of one frame as a PPM.
    //
    ...
    // CW_VK_SNAP_FRAME=N picks the frame. It was a hardcoded 600 for as long as the
    ...
    // CW_CAPTURE_KEY=<dir> — ONE PRESS, ONE PLACE, EVERY ARTIFACT, into one directory.
    //
    ...
    static const char* captureDir = Env("CW_CAPTURE_KEY");
    static const bool captureDirReady = [] {
        if (const char* d = Env("CW_CAPTURE_KEY"))
        {
            std::error_code ec;
            std::filesystem::create_directories(d, ec);
            fprintf(stderr, "[vk] CW_CAPTURE_KEY armed: press F9 to capture the picture, "
    ...
    static const char* snapDir = captureDir ? captureDir : Env("CW_VK_SNAP_DUMP");
    static const uint64_t snapFrame =
        Env("CW_VK_SNAP_FRAME") ? strtoull(Env("CW_VK_SNAP_FRAME"), nullptr, 10) : 600;
    ...
    // number chosen before the run started: a fixed `CW_VK_SNAP_FRAME` is a fine trigger
    ...
    if ((snapKey || censusFrameHit) && censusPath && !R->drawCensusFrame)
    {
        ...
        // CW_VK_DRAW_ID=1 — THE CENSUS FRAME ITSELF paints draw indices instead of
        ...
        if (EnvOn("CW_VK_DRAW_ID") && R->drawIdModule)
        {
            R->drawIdArmed = true;
    ...
    if (snapKey && !snapDir && !censusPath)
    {
        ...
        {
            complained = true;
            fprintf(stderr, "[vk] F9 pressed but neither CW_VK_SNAP_DUMP nor "
                            "CW_VK_DRAW_CENSUS is set — nothing was dumped\n");
        }
    }
    ...
    // capture is that its three views are of the same moment. So under CW_CAPTURE_KEY the
    // press arms the dump for the next frame like the other two; the standalone
    // CW_VK_SNAP_DUMP path keeps its old immediate behaviour, which several recorded
    ...
    // And the fixed-frame dump is OFF under CW_CAPTURE_KEY: that variable means "the
                    ...
                    snapKeyNow || drawIdNow))
    {
        ...
        // exactly like a renderer that had no snapshots to give. `CW_SHADER_DUMP` had this
        ...
        for (const auto& [dest, snap] : R->snapshots)
        {
            ...
            // The FRAME is in the name because CW_VK_SNAP_ON_BLACK can fire on more
            ...
            if (!f)
            {
                ...
                {
                    complained = true;
                    fprintf(stderr, "[vk] CW_VK_SNAP_DUMP cannot write %s — NO snapshots "
    ...
    static const uint64_t statsEvery =
        Env("CW_VK_STATS") ? std::max(1L, strtol(Env("CW_VK_STATS"), nullptr, 10)) : 0;
    if (statsEvery && (R->frame % statsEvery) == 0)
        VkRenderer_DumpStats();

    // CW_VK_PROFILE=N — the frame's CPU time by phase, every N seconds.
    //
    ...
    if (g_profileOn)
    {
        ...
        static double period =
            Env("CW_VK_PROFILE") ? std::max(1.0, atof(Env("CW_VK_PROFILE"))) : 5.0;
        ...
        if (dt >= period)
        {
            ...
            if (g_prof.frameStats)
                fprintf(stderr,
                        "[vkprof]   CW_VK_FRAME_STATS itself: %.2f ms/frame (%.1f%% of "
            ...
            // THE SWAPCHAIN ARM'S OWN COUNTER (CW_VK_SWAPCHAIN=1). An arm with no
            ...
            // code path actually moves.
            {
                ...
                // subtracted anywhere. CW_VK_PROFILE_EXTRA_SCOPES=N adjudicates it: it
            ...
            // rather than a saving. CW_VK_NO_PIPELINE_CACHE1=1 is the control arm.
            {
            ...
            fprintf(stderr,
                    ...
                    (dBulk + dSlow) ? 100.0 * double(dBulk) / double(dBulk + dSlow)
                                    : 0.0);
            // Under CW_PM4_VERIFY_BULK_REGS this must stay at zero. Printed only when
            ...
            // alongside because the comparison is only exact while one thread walks.
            {
                ...
                if (bad || getenv("CW_PM4_VERIFY_COUNTERS"))
                    fprintf(stderr,
            ...
            if (g_constMemoHits + g_constMemoMisses)
            {
                ...
                fprintf(stderr,
                        ...
                        frames ? double(g_constMemoHits) * 4096.0 / double(frames) / 1048576.0
                               : 0.0,
                        g_constMemoOff ? " [CW_VK_NO_CONST_MEMO: the copy runs every draw]"
                                       : "");
                fprintf(stderr,
            ...
            if (g_flatCacheLookups || g_flatCacheOff)
            {
                fprintf(stderr,
                        ...
                        g_flatCacheOff ? " [CW_VK_NO_FLAT_CACHE: the std::unordered_map is serving]" : "");
                if (g_flatGrows)
                    fprintf(stderr,
            ...
            if (R->persistOn)
            {
                ...
                // whose price is unknown is the thing that kept Case Zero's item 00c
                // parked for two parts — and on THIS port the number is unmeasured, so
                // this line is the first place it will ever appear.
                fprintf(stderr,
...
bool VkRenderer_D3DInit()
{
    ...
    if (g_active)
    {
        // The PM4 feed initialized first (CW_VKDRAW). d3d_draw.cpp refuses the
...
void VkRenderer_DumpStats()
{
    ...
    // operator's A/B harness deliberately runs without `CW_VK_PROFILE` (it costs 2-4 ms a
    ...
    fprintf(stderr,
            ...
            (unsigned long long)g_flatGrows, double(g_flatGrowNs) / 1e6,
            double(g_flatGrowWorstNs) / 1e6,
            g_flatCacheOff ? " [CW_VK_NO_FLAT_CACHE: no flat table was in use]" : "");
    ...
    // Printed unconditionally, including on the CW_VK_NO_STATE_CACHE arm where every
    ...
    if (g_texGuardStats.hits)
    {
        ...
        // nothing in this runtime had ever measured.
        {
            ...
            fprintf(stderr,
                    ...
                    g_texGuardEveryFetch ? "  [CW_VK_TEX_GUARD_EVERY_FETCH — expect 0]"
                                         : "");
        }
