...
// game's: Case Zero's `sub_8284D5C0` (addresses in this file are the sibling's) builds it from `VdGetCurrentDisplayGamma`'s answer, and for the
...
// lookup.
//
// It is an ARM (`CW_VK_GAMMA_RAMP=1`), OFF by default, because the measurement that
