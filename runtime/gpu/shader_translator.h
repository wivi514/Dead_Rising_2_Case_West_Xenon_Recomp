...
// which remains the DEV tool (arm caches via CW_DXC_DEFINES/CW_HLSL_PATCH still build
// there). The two implementations are kept honest by one gate, run after any change to
// either: `cw_runtime --translate-shaders <ucode_dir> <out>` must reproduce every .spv
...
// The whole-directory driver behind `cw_runtime --translate-shaders <in> <out>`:
...
// the honest answer. CW_TRANSLATE_KEEP_HLSL=<dir> keeps the intermediates.
