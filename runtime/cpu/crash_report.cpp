...
void Report(int sig, const void* faultAddr, unsigned long long hostPc)
{
    ...
    //     addr2line -f -C -e runtime/build/cw_runtime <host pc>
...
//
// CW_WIN_FIRSTCHANCE=1 adds a vectored handler that only LOGS, so the traffic this used
...
extern "C" void CzInstallCrashReporter()
{
    ...
    if (const char* fc = getenv("CW_WIN_FIRSTCHANCE"); fc && *fc != '0')
    {
        fprintf(stderr, "[seh] CW_WIN_FIRSTCHANCE=%s — logging every first-chance "
