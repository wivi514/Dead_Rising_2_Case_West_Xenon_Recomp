...
void AdvanceFrame()
{
    ...
    virtual_ticks += CW_TIMEBASE_HZ / 60;
}

bool init()
{
    host_hz = calibrate();
    if (getenv("CW_DETERMINISTIC_CLOCK"))
    {
        deterministic = true;
        ...
        virtual_ticks = CW_TIMEBASE_HZ * 10;
        fprintf(stderr,
                "[timebase] CW_DETERMINISTIC_CLOCK: the guest clock advances %llu ticks "
                "per PRESENTED FRAME and does not track wall time. This MANUFACTURES a "
                "condition — never use it for a gate run.\n",
                (unsigned long long)(CW_TIMEBASE_HZ / 60));
    }
    return host_hz != 0;
}
