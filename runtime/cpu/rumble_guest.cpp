// THE TITLE'S RUMBLE, ON THE CONSOLE'S CLOCK (Case Zero part 108, imported in Case West
// part 11) — and the probe that found it.
//
// PROVENANCE ON THIS IMAGE. The four hooked/named functions were RE-DERIVED here by
// byte-shape search over default_image.bin (the sibling's instruction sequences, found
// once each) and confirmed by disassembly instruction for instruction: the tick
// sub_828003D8 (Case Zero sub_82805A58; same `lbz flag; cmplwi 1`, `li r22,8`, 16-entry
// port loop, table walk), SetMotor sub_82801130 (sub_828067F8; `cmpwi r4,2; bgelr;
// addi r11,r4,0x923; stfsx`), Send sub_82801160 (sub_82806828; two motors scaled and
// packed, then `bl` the XamInputSetState wrapper sub_825AA420, their sub_825D7AC8) and
// StopAll sub_82801210 (sub_828068D8). The effect table is at 0x82AF0880 here (theirs
// 0x82AD66B8); the pad vtable has one extra slot, so SetMotor is vt+0x58 and Send
// vt+0x5C (theirs +0x54/+0x58). The call chain above it is this image's own
// (tools/guest_callers.py). The remaining addresses in this header — the register/
// unregister pair, the definition ctor and the two GetLevel forms — are Case Zero's
// read-out of the effect system and were NOT re-derived here; nothing hooks them.
//
...
// to ONE function, sub_828003D8, the title's rumble manager tick, which the frame loop
// calls once per FRAME (sub_828008C8 <- sub_82491868 <- sub_824A46A8). Read in full:
//
//   * a 100-entry effect table at 0x82AF0880, 16 bytes each: {ticks remaining, loop
...
//     wins outright, equal priorities max), SetMotor (vt+0x58 here, sub_82801130) per
//     motor, Send (vt+0x5C here, sub_82801160 -> XamInputSetState), THEN every entry's
...
// the console; at 60 (a shipped configuration, CW_FPS_CAP=60) it gives the 30-fps
// authoring rather than the console's own halved effects at 60. `CW_RUMBLE_TICK_HZ=N`
// sets the rate; `=0` is the control arm (the tick per frame, the runtime as it was
// on the day the operator reported it).
//
// The probe (CW_RUMBLE_TRACE): sub_82801130 is SetMotor(index r4, level f1 in 0..1);
// sub_82801210 is StopAll. Armed by the same switch as the host side, so one env var
...
// ~/DR2CW-troubleshooting/<part>/watched_play.sh (Case Zero's recipe), which kills the game and keeps the
...
extern "C" PPC_FUNC(__imp__sub_82801130);
extern "C" PPC_FUNC(__imp__sub_82801210);
extern "C" PPC_FUNC(__imp__sub_828003D8);

namespace
{
bool Armed()
{
    static const bool armed = getenv("CW_RUMBLE_TRACE") != nullptr;
    return armed;
}
double Now()
{
...
PPC_FUNC(sub_82801130)
{
    ...
    __imp__sub_82801130(ctx, base);
}

PPC_FUNC(sub_82801210)
{
    if (Armed())
    {
        const double t = Now();
...
double TickHz()
{
    static const double hz = [] {
        const char* e = getenv("CW_RUMBLE_TICK_HZ");
        const double v = e ? atof(e) : 30.0;
        if (e)
            fprintf(stderr, "[rumble] CW_RUMBLE_TICK_HZ=%s: the title's rumble tick runs %s\n", e,
...
PPC_FUNC(sub_828003D8)
{
    const double hz = TickHz();
    if (hz <= 0.0)
    {
        __imp__sub_828003D8(ctx, base);
        return;
    }
    ...
    {
        announced = true;
        fprintf(stderr, "[rumble] the title's rumble tick (sub_828003D8) runs at %.0f Hz of "
