...
//
// The FLOOR came from Case Zero cc05d05 (their part 100, the operator's request there):
// the bare subtract-reserve-and-commit formula gave a 6-core machine 1 worker and a
// 4-core machine 0, leaving the mid-range CPUs most real players run almost serial. The
// floor guarantees a machine with cores to spare actually uses them. The fully serial
// path is still reachable and still the control arm — CW_WORKERS=0.
//
...
// (gotcha 344).
//
// ONE KNOB. `CW_WORKERS=N` overrides the whole budget and `CW_WORKERS=0` forces the serial
...
// already exist (`CW_VK_GUARD_WORKERS`, `CW_VK_NO_PARALLEL_GUARD`) still win where they
...
// "CW_VK_GUARD_WORKERS"); pass nullptr for none. Idempotent per pool name: asking twice
