// The Draw Thread's FENCE wait, PARKED instead of spun (Case Zero perf plan part 107,
// item 2; imported in Case West part 11).
//
// PROVENANCE ON THIS IMAGE: both hooked functions were RE-DERIVED here by byte-shape
// search (the `lwz r11,0x2a90(r3); lwz r10,0x2a9c(r3)` pair and the `mtctr 4` /
// eight `mr r31,r31` body occur exactly once each in default_image.bin) and read
// instruction for instruction against the sibling's: the loop sub_825B5FB8 is Case
// Zero's sub_82845160, the body sub_825B7668 their sub_8283C6C8, the nested helper
// sub_825B6DC0 their sub_82845F68. Every device-struct offset (0x2A90, 0x2A9C, 0x2ABD,
// 0x2A88, 0x2B00, 0x3460) and the 0x1388 hang check are identical.
//
// WHY THIS EXISTS. The title's Draw Thread throttles itself against the GPU by waiting
// for a fence: sub_825B5FB8 loops until the fence-completion word — the first dword of
...
// number, calling sub_825B7668 between checks, whose body is 32 `db16cyc` SMT-yield
...
//
// THE SHAPE. Two hooks, both call-through: sub_825B5FB8 publishes what the wait is FOR
// (the device struct and the target) in a thread-local, and sub_825B7668 — the body the
...
//
// CW_FENCE_PARK=0 is the control arm: both hooks call straight through and the thread
// spins exactly as it has since phase 1. CW_FENCE_PARK_SPIN_US=N sets the paused-spin
// phase (default 50 us) — 0 parks immediately. CW_FENCE_PARK_TRACE=N prints the first
...
{
    uint64_t bodyCalls = 0;      // sub_825B7668 invocations under a published wait
