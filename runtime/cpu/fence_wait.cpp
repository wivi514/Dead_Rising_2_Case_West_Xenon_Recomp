...
#include <immintrin.h>
#define CW_CPU_PAUSE() _mm_pause()
#elif defined(__aarch64__)
#define CW_CPU_PAUSE() __asm__ __volatile__("yield")
#else
#define CW_CPU_PAUSE() ((void)0)
#endif
...
// example).
extern "C" PPC_FUNC(__imp__sub_825B5FB8);
extern "C" PPC_FUNC(__imp__sub_825B7668);
...
namespace
{
// The device-struct fields the wait reads, from sub_825B5FB8's own loop (phase1-notes
...
// CW_FENCE_PARK_TRACE=N: print the first N episodes on both sides, with the values the
...
int TraceLeft()
{
    static std::atomic<int> left{ [] {
        const char* e = getenv("CW_FENCE_PARK_TRACE");
...
unsigned SpinUs()
{
    static const unsigned us = [] {
        if (const char* e = getenv("CW_FENCE_PARK_SPIN_US"))
...
bool FenceWait_Enabled()
{
    static const bool on = [] {
        const char* e = getenv("CW_FENCE_PARK");
        ...
        if (enabled)
            fprintf(stderr,
                    ...
                    "(CW_FENCE_PARK=0 restores the spin)\n",
                    SpinUs());
        else
            fprintf(stderr, "[fencewait] the Draw Thread's fence wait SPINS "
                            "(CW_FENCE_PARK=0 — the pre-part-107 behaviour)\n");
        return enabled;
    }();
    return on;
}
...
// rather than set and cleared, because sub_825B6DC0 both calls this function and is
// called from inside it (the r7 path), so the wait nests.
PPC_FUNC(sub_825B5FB8)
{
