# Dead Rising 2: Case West — native PC recompilation (Steam Deck build)
...
It is v1.1.0 with three changes, all of them for the Deck (the same three Case Zero's
...
`~/.local/share/Dead Rising 2 Case West/`.
...
Do this in **Desktop Mode** the first time — the first run has to unpack 1.2 GB and
...
3. Run `./cw_runtime` — from a terminal, or by double-clicking it in Dolphin.
...
Once it runs in Desktop Mode: right-click `cw_runtime` -> *Add to Steam*, then switch
...
```
./cw_runtime --diag                      # writes cw_diag.txt, prints your GPU/driver
./cw_runtime                             # let it crash
LD_DEBUG=libs ./cw_runtime 2> ld.log     # the tail names the library it died loading
```

Then send **`cw_runtime.log`** (written next to the executable; the previous run is
kept as `cw_runtime.log.1`), **`cw_diag.txt`** and **`ld.log`**. Say what you saw —
...
|---|---|
| `CW_LAUNCHER=1 ./cw_runtime` | puts the launcher back. If this crashes and the plain run does not, the launcher was the whole problem. |
| `CW_VKDRAW=0 ./cw_runtime` | starts with no renderer at all — a blank window, on purpose. If even this crashes, the fault is before the graphics. |
...
|---|---|
| `CW_VK_NO_BIND_BATCH=1` | turns off batched vertex-buffer binding |
| `CW_VK_NO_DEVICE_PFN=1` | turns off the direct driver-function table |
...
* `cw_defaults.env` — default settings applied when not set in your environment.
  In this build it sets `CW_LAUNCHER=0` (no launcher) and `CW_VK_RES=1280x800` (the
