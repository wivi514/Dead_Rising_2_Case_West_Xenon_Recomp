...
# and 6.4-6.7 ms under CW_VK_NO_DODRAW=1 — so OUR renderer's presence costs the title's
...
#
# Two arms on the same binary: normal, and CW_VK_NO_DODRAW=1 (the guest floor). Same route
...
( for _ in $(seq 1 $((STAT_SECS * 3 * 4))); do
      ...
      for t in /proc/$PID/task/*; do
          c=$(cat "$t/comm" 2>/dev/null) || continue
          case "$c" in cw-pump|cw-draw|cw-guard*) 
              u=$(awk '{print $14+$15}' "$t/stat" 2>/dev/null || echo 0)
