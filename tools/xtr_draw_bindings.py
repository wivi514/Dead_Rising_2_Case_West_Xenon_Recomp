...
our own dump filenames: run with `--self-test` and it hashes `~/DR2CW-troubleshooting/
...
def main():
    ...
    ap.add_argument('--max-draws', type=int, default=12,
                    help='how many draws to list (default 12, by descending vertex count). '
                         'The default silently hid the Bink YUV draws on 2026-08-15 -- '
                         'a 4-vertex fullscreen quad is at the BOTTOM of a vertex-count sort, '
                         'so raise this or use --csv before concluding a texture is absent.')
    ap.add_argument('--csv')
    ...
    if args.self_test:
        import glob
        ok = bad = 0
        for p in glob.glob(os.path.expanduser('~/DR2CW-troubleshooting/ucode-dumps/*.ucode')):
    ...
    for i, n, names, tex, prim, cc, aref, bl, dc, si in sorted(draws, key=lambda d: -d[1]):
        if n < args.min_verts or shown >= args.max_draws:
            continue
        shown += 1
