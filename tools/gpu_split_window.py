#!/usr/bin/env python3
"""Read CW_VK_GPU_PASSES dumps out of a log and print the split for a WINDOW, not the run.

WHY THIS EXISTS. The per-region GPU split (`CW_VK_GPU_PASSES=1`) prints CUMULATIVE
...
several thousand 60-draw frames. `CW_VK_STATS=N` re-prints the whole counter block every N
...
HDR = re.compile(r"GPU per-region split \(CW_VK_GPU_PASSES\) over (\d+) frames — ([\d.]+) ms/frame measured, ([\d.]+) ms attributed")
...
def main():
    ...
    if not dumps:
        print("no GPU per-region split dump in", sys.argv[1], "(CW_VK_GPU_PASSES=1 and either "
              "an exit dump or CW_VK_STATS=N are needed)")
        sys.exit(1)
