...
# ~9,000-draw crowd (config/part80_crowd_route.seq, the same CW_FAKE_PRESS_SEQ), headless,
...
#       -SoakMs 70000 -Env @{CW_GUEST_PIN="0"}
#
# -Root is where cw_runtime.exe lives (the game's assets resolve from the exe, HostPaths);
...
$line = (Get-Content $Seq | Where-Object { $_ -match '^CW_FAKE_PRESS_SEQ=' } | Select-Object -Last 1)
...
$env:CW_NO_WINDOW = "1"; $env:CW_VKDRAW = "1"; $env:CW_LAUNCHER = "0"; $env:CW_FPS_CAP = "500"; $env:CW_FPS_LOG = "10"
$env:CW_VK_RES = "1920x1080"; $env:CW_NO_AUDIO_OUT = "1"; $env:CW_FAKE_START_MS = "100"
...
$env:CW_DEBUG_MENU = "1"
$env:CW_DEBUG_FLAGS = "CHUCK GOD MODE,DISABLE DEATH SEQUENCE,ZOMBIES IGNORE ALL HUMANS"
$env:CW_FAKE_PRESS_SEQ = $seq
foreach ($k in $Env.Keys) { Set-Item -Path "Env:$k" -Value $Env[$k] }
Set-Location $Root
$log = "$o\$Tag.err.log"
$t0 = Get-Date
$p = Start-Process -FilePath "$Root\cw_runtime.exe" -WorkingDirectory $Root -RedirectStandardError $log -RedirectStandardOutput "$o\$Tag.out.log" -PassThru -WindowStyle Hidden
