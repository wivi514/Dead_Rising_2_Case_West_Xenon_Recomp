...
#      under the same path the route copies to (`cw_runtime_crowd`), then moved.
...
#
# NO CW_VK_PROFILE (4-5 ms a frame, inverts the regime — gotcha 454). `perf` needs no
...
( OUT="$OUT" RES="$RES" SOAK="$SOAK" BIN_SRC="$BIN_SRC" "$ROOT/tools/part80_crowdroute.sh" "$TAG" \
      CW_FPS_LOG=10 "$@" > "$OUT/$TAG.route.out" 2>&1 ) &
RUNNER=$!
...
for _ in $(seq 1 90); do
    for p in $(pgrep cw_runtime 2>/dev/null); do
        c=$(cat "/proc/$p/comm" 2>/dev/null) || continue
        case "$c" in cw_runtime_crow*) PID=$p ;; esac
    done
    [ -n "$PID" ] && break
    sleep 1
done
[ -z "$PID" ] && { echo "!! no cw_runtime_crowd appeared"; wait $RUNNER; exit 3; }
LOG=""
...
if [ "$PERF" = 1 ]; then
    ...
    # the ARCHIVED binary after the route has overwritten cw_runtime_crowd with the
    ...
    SYMFS="$OUT/$TAG.symfs"
    mkdir -p "$SYMFS$(dirname "$ROOT/runtime/build/cw_runtime_crowd")" "$SYMFS/usr"
    ln -sfn "$OUT/$TAG.bin" "$SYMFS$ROOT/runtime/build/cw_runtime_crowd"
