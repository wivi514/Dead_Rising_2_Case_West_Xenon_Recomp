...
int main(int argc, char** argv)
{
    ...
    fprintf(map, "# image_base=0x%llX image_size=0x%X entry=0x%llX\n",
            ...
            (unsigned long long)image.entry_point);
    // The legend inherited from the Case Zero copy of this file read "1=code 2=data
    // 4=bss" and was WRONG in the one way that matters: XenonUtils/section.h defines
    // SectionFlags_Data = 1 and SectionFlags_Code = 2, and nothing else. Nobody noticed
    // in Case Zero because that title has exactly one code section, `.text`, which
    // prints 2 — read as "data" by the legend and as "the code section" by everyone
    // looking at it, so the two errors cancelled. Case West has TWO code sections
    // (`.text` and `BINK`, the statically linked RAD movie decoder) and the legend
    // would have said both were data.
    fprintf(map, "# flags: 1=data 2=code (XenonUtils/section.h SectionFlags)\n");
