#!/bin/bash
# Part 117 gates on the two-core pump (CW_PUMP_SPLIT=1) — every standing gate the part-116
...
#
# Usage:  ARM="CW_PUMP_SPLIT=1 CW_WAITANY_WAKE=1" tools/part117_gates.sh
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT" || exit 2
ARM="${ARM:-CW_PUMP_SPLIT=1 CW_WAITANY_WAKE=1}"
...
( cd runtime/build && env $ARM CW_NO_WINDOW=1 CW_VKDRAW=1 timeout 150 ./cw_runtime > "$OUT/a5.log" 2>&1 )
python3 tools/kernel_call_diff.py --xenia "Xenia logs/A5_highfreq_boot/cw_run5.log" \
...
env $ARM CW_VK_SYNC_VALIDATION=1 CW_VK_RES=1280x720 PRESSMS=9000 SECS=45 TIMEOUT=420 \
...
env $ARM CW_VK_SYNC_VALIDATION=1 CW_VK_BARRIER_POISON=1 CW_VK_RES=1280x720 PRESSMS=9000 SECS=45 TIMEOUT=420 \
...
echo "--- 5. explorer soak, 10 min, split on, CW_WAIT_TRACE=1"
( cd runtime/build && env $ARM CW_NO_WINDOW=1 CW_VKDRAW=1 CW_DEBUG_MENU=1 CW_AUTOCHUCK=EXPLORER \
    CW_FAKE_START_MS=8000 CW_FAKE_PRESS_SEQ=F2,START,WAITJUMP,NONE,DOWN,A,NONE CW_WAIT_TRACE=1 CW_FPS_LOG=30 \
    timeout 600 ./cw_runtime > "$OUT/soak.log" 2>&1 )
