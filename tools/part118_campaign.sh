...
#   B  CW_HAVOK_WORKERS=4         (main + 4 instead of main + 2)
#   C  B + CW_VK_GUARD_NTA=1      (the guard's sweep kept out of L2/L3)
...
for i in $(seq 1 "$N"); do
    ...
    BIN_SRC="$BIN" "$ROOT/tools/part80_crowdroute.sh" "c1B_hk4_$i" CW_HAVOK_WORKERS=4
    BIN_SRC="$BIN" "$ROOT/tools/part80_crowdroute.sh" "c1C_hk4nta$i" CW_HAVOK_WORKERS=4 CW_VK_GUARD_NTA=1
done
echo "C1 DONE"
