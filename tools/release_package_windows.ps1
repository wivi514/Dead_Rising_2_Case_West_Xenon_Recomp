...
# the sibling Case Zero repo's docs/windows-build-setup.md for the tree it assumes under C:\cz).
#
...
$exe = Join-Path $BuildDir "cw_runtime.exe"
...
if (Test-Path $stageAssets) {
    ...
    if ($player -or $pkgFiles) {
        $keep = Join-Path $OutDir "CaseWestRecomp.assets.keep"
...
$dlls = @(
    ...
    # this very machine. The build is configured against C:\cw\curl (build_cw_xlive.ps1).
...
CW_VKDRAW=1
CW_LAUNCHER=1
"@ | Set-Content -Encoding ascii (Join-Path $Stage "cw_defaults.env")
...
$smoke = & (Join-Path $Stage "cw_runtime.exe") --smoke 2>&1 | Out-String
...
# Part 105: the runtime writes cw_runtime.log / cw_diag.txt beside its data root, which
...
foreach ($leftover in @("cw_runtime.log", "cw_runtime.log.1", "cw_diag.txt", "cw_diag.txt.1")) {
