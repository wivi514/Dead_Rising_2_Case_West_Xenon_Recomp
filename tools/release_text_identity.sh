...
for d in "$DEV" "$REL"; do
    [ -x "$d/cw_runtime" ] || fail "no executable at $d/cw_runtime"
done
...
for arm in dev rel; do
    case $arm in dev) src=$DEV/cw_runtime ;; rel) src=$REL/cw_runtime ;; esac
...
# those: it was CW_FFMPEG_PREFIX set in one tree and not the other, because a release
...
cachediff=""
# CW_BUNDLE_RPATH is in this list and it surprised the first run of the matched
...
for var in CMAKE_BUILD_TYPE CMAKE_CXX_COMPILER CMAKE_CXX_FLAGS CMAKE_CXX_FLAGS_RELEASE \
           CMAKE_CXX_FLAGS_RELWITHDEBINFO CW_FFMPEG_PREFIX CW_SDL2_PREFIX CW_DEBUG_TUS \
           CW_BUNDLE_RPATH CMAKE_INTERPROCEDURAL_OPTIMIZATION; do
