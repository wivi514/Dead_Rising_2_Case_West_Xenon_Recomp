...
def analyse_bctr(image, bctr_addr, window, code_lo, code_hi):
...
def analyse_bctr_offset_table(image, bctr_addr, window, code_lo, code_hi):
...
# SectionFlags_Code, from XenonRecomp's XenonUtils/section.h. Data is 1, code is 2 —
# NOT the "1=code" the sidecar's legend claimed until 2026-08-15.
SECTION_FLAG_CODE = 2


def code_ranges(args):
    """Return the [(lo, hi), ...] guest-VA ranges to scan.

    WHY THIS IS NOT A PAIR OF DEFAULTS ANYMORE
    ------------------------------------------
    This tool arrived from the Case Zero port with that title's `.text` bounds baked in
    as the default `--lo`/`--hi` (0x82150000..0x829C3564). Run unchanged against Case
    West it printed a confident summary — 204 tables, "unhandled: 0" — for a range that
    stopped 0x86F0 short of this title's `.text` AND omitted its second code section
    entirely. Nothing in the output said so; the header line is the only place the
    range appears at all. That is gotcha 3 with the detector's own bounds as the zero.

    So the range now comes from the image's own section map, which
    `tools/xex_image_dump` writes next to the flat image, and EVERY section flagged as
    code is scanned. Case West has two: `.text` and `BINK` — the statically linked RAD
    movie decoder, which carries one absolute table of its own. XenonRecomp recompiles
    both (recompiler.cpp iterates all SectionFlags_Code sections), so a table missing
    from the config is a live dispatch with no lowering, in either of them.

    `--lo`/`--hi` still override, for scanning a single range by hand.
    """
    if args.lo or args.hi:
        if not (args.lo and args.hi):
            sys.exit("--lo and --hi must be given together")
        return [(int(args.lo, 16), int(args.hi, 16))]

    path = Path(args.sections) if args.sections else Path(args.xex + ".sections")
    if not path.exists():
        sys.exit(f"no section map at {path} — run tools/xex_image_dump first, "
                 f"or pass --lo/--hi explicitly")

    ranges = []
    for line in path.read_text().splitlines():
        if not line.strip() or line.startswith("#"):
            continue
        # The name column can contain spaces (Case West ships a section literally named
        # "BINKCONS )"), so split from the RIGHT: the last three fields are fixed.
        parts = line.rsplit(None, 3)
        if len(parts) != 4:
            continue
        _name, base, size, flags = parts
        if int(flags, 0) & SECTION_FLAG_CODE:
            ranges.append((int(base, 16), int(base, 16) + int(size, 16)))
    if not ranges:
        sys.exit(f"{path} lists no code sections — check the flags legend")
...
def main():
    ...
    ap.add_argument("--lo", help="code section start (guest VA); overrides --sections")
    ap.add_argument("--hi", help="code section end (guest VA); overrides --sections")
    ap.add_argument("--sections", default=None,
                    help="section map from tools/xex_image_dump (default: <image>.sections). "
                         "Every section flagged as CODE is scanned.")
    ap.add_argument("--stats", action="store_true", help="summary only, no TOML")
    args = ap.parse_args()

    img = Image(args.xex, int(args.image_base, 16))
    ranges = code_ranges(args)
    ...
    tables.sort(key=lambda t: t["base"])
