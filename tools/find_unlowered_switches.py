...
TABLES = ROOT / "config/CaseWest_switch_tables.toml"
PPC = ROOT / "ppc"
IMAGE_BASE = 0x82000000

# The code ranges to sweep for `bctr`, read from the image's own section map rather
# than hardcoded. This was `0x82150000, 0x82150000 + 0x873564` — Case Zero's `.text` —
# when the file was copied here on 2026-08-15. Case West's `.text` is 0x87BC54 and it
# has a SECOND code section (`BINK`, the statically linked RAD movie decoder), so the
# inherited constant would have skipped 0x86F0 of `.text` plus all of BINK while this
# gate still printed "0 defects". That is the failure mode this gate exists to prevent,
# committed by the gate itself.
SECTION_FLAG_CODE = 2   # XenonUtils/section.h: Data = 1, Code = 2


def _code_ranges():
    p = ROOT / "assets/game/default_image.bin.sections"
    if not p.exists():
        sys.exit(f"no section map at {p} — run tools/xex_image_dump first")
    out = []
    for line in p.read_text().splitlines():
        if not line.strip() or line.startswith("#"):
            continue
        # Split from the RIGHT — a section name may contain spaces ("BINKCONS )").
    ...
    if not out:
        sys.exit(f"{p} lists no code sections — check the flags legend")
    return sorted(out)


CODE_RANGES = _code_ranges()
TEXT_LO = min(lo for lo, _ in CODE_RANGES)
TEXT_HI = max(hi for _, hi in CODE_RANGES)
...
def main():
    ...
    total = 0
    candidates = []
    # Per range, not TEXT_LO..TEXT_HI, so the inter-section padding between `.text` and
    # `BINK` is never decoded as instructions.
