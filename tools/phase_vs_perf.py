...
`CW_VK_PROFILE` prints a table of named phases that reads like a decomposition of the
...
project, because the route script re-links `cw_runtime_crowd` on the next build. The
...
def recorded_main_dso(perf_data):
    ...
    for line in out.splitlines():
        ...
        if len(parts) == 2 and "cw_runtime" in parts[1]:
...
def perf_symbols(perf_data, binary=None):
    ...
    if binary:
        ...
        if not rec:
            sys.exit("!! no cw_runtime DSO in the build-id list; wrong perf.data?")
...
def read_log(path, floor):
    ...
    if not rows:
        sys.exit(f"!! no CW_VK_PROFILE windows at >= {floor} draws in {path}")
...
def self_test():
    ...
    the archive carrying both `perf` and `CW_VK_PROFILE`, which is itself a finding and
