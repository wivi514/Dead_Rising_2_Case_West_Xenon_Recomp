...
#   nodd          CW_VK_NO_DODRAW=1 — the per-draw renderer deleted, the wall IS the guest
...
#   ENVB="CW_VK_TEXMEMO=1 CW_VK_SCOPED_SHARED_ZERO=1" tools/part116_ab.sh base BIN bundle BIN
set -u
...
for i in $(seq 1 "$N"); do
    for kind in $KINDS; do
        extra=""
        [ "$kind" = nodd ] && extra="CW_VK_NO_DODRAW=1"
