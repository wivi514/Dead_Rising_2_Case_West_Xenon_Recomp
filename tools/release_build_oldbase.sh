...
#         CW_OLDBASE_SKIP_DEPS=1 ...                  reuse thirdparty/oldbase/{sdl2,ffmpeg-lgpl}
set -euo pipefail

ROOT=$(cd "$(dirname "$0")/.." && pwd)
# The image is the sibling port's (Case Zero's tools/release/oldbase/Containerfile is
# byte-identical to ours after the rename): on a box that built cz-oldbase:jammy already,
# `podman tag cz-oldbase:jammy cw-oldbase:jammy` saves the 1 GB rebuild.
IMAGE=${CW_OLDBASE_IMAGE:-cw-oldbase:jammy}
...
[ -f "$DXC_SRC" ] || fail "no libdxcompiler.so at $DXC_SRC (set CW_DXC_LIB)"
...
# Source tarballs seeded from either port's host build cache, when present.
for d in cw cz; do
[ -f /var/tmp/$d-sdl2-build/SDL2-2.32.10.tar.gz ] && cp -n /var/tmp/$d-sdl2-build/SDL2-2.32.10.tar.gz "$OB/work/sdl2/" || true
[ -f /var/tmp/$d-ffmpeg-build/ffmpeg-8.1.2.tar.xz ] && cp -n /var/tmp/$d-ffmpeg-build/ffmpeg-8.1.2.tar.xz "$OB/work/ffmpeg/" || true
done
...
RUN=(podman run --rm -i
     ...
     -e HOME="$HOMEDIR" -e CW_DXC_LIB=/opt/dxc/libdxcompiler.so
     -e CW_SDL2_WORK="$OB/work/sdl2" -e CW_FFMPEG_WORK="$OB/work/ffmpeg"
     -e CW_SDL2_PREFIX="$OB/sdl2" -e CW_FFMPEG_PREFIX="$OB/ffmpeg-lgpl"
...
if [ -z "${CW_OLDBASE_SKIP_DEPS:-}" ] || [ ! -f "$OB/sdl2/lib/libSDL2-2.0.so.0" ] || [ ! -f "$OB/ffmpeg-lgpl/lib/libavcodec.so" ]; then
...
cfg() {
    cmake -S runtime -B "$1" -G Ninja -DCMAKE_BUILD_TYPE="$2" \
        -DCW_FFMPEG_PREFIX="$OB/ffmpeg-lgpl" -DCW_SDL2_PREFIX="$OB/sdl2" -DCW_BUNDLE_RPATH=ON \
...
echo
echo "==> NEXT"
echo "    tools/release_package_appimage.sh                     # wrap dist/CaseWestRecomp"
echo "    # AT THE FLOOR (ubuntu:22.04 = glibc 2.35, the base): both must print GATE PASSED"
echo "    tools/release_gate_clean_container.sh dist/CaseWestRecomp docker.io/library/ubuntu:22.04"
echo "    tools/release_gate_clean_container.sh dist/CaseWestRecomp-linux-x86_64.AppImage docker.io/library/ubuntu:22.04"
...
echo "    tools/release_gate_clean_container.sh dist/CaseWestRecomp quay.io/rockylinux/rockylinux:9-minimal"
