...
float at 0x82A578D0 instead of the clock — the same two globals `CW_DEBUG_FLAGS` and
...
# CASE WEST: THE FIVE GUEST ADDRESSES BELOW ARE CASE ZERO'S AND HAVE NOT BEEN RE-DERIVED
# ON THIS IMAGE (copied 2026-09-17 with the sibling's parts 119-120 tooling; see
# docs/imported-fixes.md §14). They are kept as the RECORD of what to look for — the
# `DISABLE TIME OF DAY` byte is in runtime/cpu/debug_tunables_table.inc under its own
# name here; the pinned-hour float, the gamma meter and the clock vtable are found the
# way the sibling's commit messages (9f42de9, 79e6b58) describe — and the script REFUSES
# to run until they are, because a poke at the sibling's address on this image writes an
# unrelated word (the port's characteristic failure: a tool copied from a sibling carries
# that sibling's constants — CLAUDE.md, "The thesis of this port").
ADDRESSES_RE_DERIVED_FOR_CASE_WEST = False
FLAG_DISABLE_TIME_OF_DAY = 0x82A57CAA   # CASE ZERO: byte; the title's debug bool
PINNED_HOUR = 0x82A578D0                # CASE ZERO: float, hours 0..24, read when the flag is set
GAMMA_METER = 0x829EDCF4                # CASE ZERO: float, the Visuals gamma meter's value
CLOCK_VTABLE = 0x820112C0               # CASE ZERO: the mission clock object's vtable (found live)
CLOCK_DAY, CLOCK_HOUR, CLOCK_MIN, CLOCK_SEC = 0x14, 0x18, 0x1C, 0x20
ANCHOR_VA = 0x820B40C8                  # CASE ZERO: where `gFinalGammaParameters` sits in THAT xex
ANCHOR = b'gFinalGammaParameters'
if not ADDRESSES_RE_DERIVED_FOR_CASE_WEST:
    sys.exit("xenia_poke.py: the guest addresses in this copy are Case Zero's — re-derive them "
             "on assets/game/default_image.bin (see the header) and flip "
             "ADDRESSES_RE_DERIVED_FOR_CASE_WEST before using it on Case West")
...
def find_base(h):
    ...
    sys.exit('guest mapping not found: no candidate base holds the XEX anchor string '
             '(is this Case West, and is it past the loader?)')
