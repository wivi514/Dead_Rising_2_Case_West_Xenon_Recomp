...
there** (decals — since fixed, see the status table — and performance), tracked in `docs/imported-fixes.md`.
...
| Shader cache | ✅ **492 shaders, 492 translated, 0 failures**, dim census 0 disagreements — grew 443 → 461 → 480 (2026-08-27/28) → **492 on a full end-to-end completion run (2026-09-05, part 8)**, the in-process first-sight translator writing each new one in-boot and the `--translate-shaders` gate confirming all 492 .spv byte-identical to the reference. **Arm `CW_SHADER_DUMP` on every session** — a miss is silent (gotcha 324). The in-process translator (import row 5) now translates a missing shader at first sight in-boot, sidecars carry `aluConsts`. Cache + ucode-dumps are gitignored (rebuilt from source); the completion run establishes coverage, not a committed blob |
