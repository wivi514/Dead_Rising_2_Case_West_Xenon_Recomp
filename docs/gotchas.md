...
> **COPIED INTO THE CASE WEST PORT ON 2026-08-15, VERBATIM, AT 315 ENTRIES.**
> Entries 1–315 were paid for in `~/GithubRepo/Fable2XenonRecomp`,
> `~/GithubRepo/Asuras_Wrath_Xenon_Recomp` and
> `~/GithubRepo/Dead_Rising_2_Case_Zero_Xenon_Recomp`, in that order; new entries
> from this port continue at **316**. Two things follow from it being a copy:
>
> - Where an entry names a Case Zero file, env var, address or count, that is a
>   *pointer into the sibling repo*, not a claim about this one. Verify before acting.
> - Case Zero is still live and will keep adding entries. This copy does not
>   auto-merge. When a session there lands something transferable, bring it across by
>   hand and keep the number.
>
> The alternative — referencing the sibling repo's copy — was rejected because a port
> has to stay independently preservable, which is `reusability.md`'s own rule.
> Session 1's own contribution is in `bootstrap-2026-08-15.md` §7 and is a straight
> restatement of gotcha 3: **a tool copied from a sibling port carries that port's
> constants, and a constant wrong in the safe direction reports a clean run over a
> smaller image.**
