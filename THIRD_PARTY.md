...
> **Copied from the Case Zero port on 2026-08-15 as this repository's starting ledger.**
> The licence declaration below applies here too, and the component analysis is the same
> analysis of the same upstream projects. What it does **not** yet cover is anything Case
> West adds on its own — in particular **Bink** (RAD Game Tools), which this title
> statically links and which Case Zero does not use at all. If the port ends up relying on
> ffmpeg's Bink decoders rather than the recompiled `BINK` section, that is a new entry to
> record **before** the first line is written, per the rule below.
